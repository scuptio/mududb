...
impl SocketConnectRequest {
    fn new(fd: RawFd, addr: crate::uring::SockAddrBuf, state: Arc<OpState<()>>) -> Self {
        Self { fd, addr, state }
    }
    ...
    pub fn addr(&self) -> &crate::uring::SockAddrBuf {
        &self.addr
    }
...
impl SocketAcceptRequest {
    fn new(fd: RawFd, state: Arc<OpState<(RawFd, SocketAddr)>>) -> Self {
        Self {
            fd,
            addr: crate::uring::SockAddrBuf::new_empty(),
            state,
        }
    }
    ...
    pub fn addr_mut(&mut self) -> &mut crate::uring::SockAddrBuf {
        &mut self.addr
    }
...
struct SocketConnectFuture {
    fd: RawFd,
    addr: Option<crate::uring::SockAddrBuf>,
...
fn socket_addr_to_raw(addr: SocketAddr) -> crate::uring::SockAddrBuf {
    crate::io::net::socket_addr_to_storage(addr).expect("socket addr to storage conversion failed")
}

pub fn raw_to_socket_addr(addr: &crate::uring::SockAddrBuf) -> RS<SocketAddr> {
...
mod tests {
    ...
    async fn socket_and_connect_enqueue_requests() {
        ...
        let create_task =
            crate::tokio::spawn(async { socket(libc::AF_INET, libc::SOCK_STREAM, 0).await });
        yield_now().await;
        ...
        let connect_task = crate::tokio::spawn(async move {
    ...
    async fn accept_recv_send_shutdown_and_close_enqueue_requests() {
        ...
        let accept_task = crate::tokio::spawn(async move { accept(&sock).await });
        yield_now().await;
        match ring.take_pending().unwrap().unwrap().1 {
        ...
        let recv_task = crate::tokio::spawn(async move {
            let mut buf = [0u8; 8];
        ...
        let send_task = crate::tokio::spawn(async move {
        ...
        let shutdown_task =
            crate::tokio::spawn(async move { shutdown(&shutdown_sock, libc::SHUT_WR).await });
        yield_now().await;
        match ring.take_pending().unwrap().unwrap().1 {
        ...
        let close_task = crate::tokio::spawn(async move { close(IoSocket { fd: 71 }).await });
        yield_now().await;
        match ring.take_pending().unwrap().unwrap().1 {
