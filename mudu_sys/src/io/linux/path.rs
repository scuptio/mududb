...
pub async fn create_dir_all(path: &Path) -> RS<()> {
    if !has_current_worker_ring() {
        return crate::io::fs::create_dir_all(path).await;
    }
...
pub async fn metadata_len(path: &Path) -> RS<u64> {
    if !has_current_worker_ring() {
        return crate::io::fs::metadata_len(path).await;
    }
...
pub async fn remove_file_if_exists(path: &Path) -> RS<()> {
    if !has_current_worker_ring() {
        return crate::io::fs::remove_file_if_exists(path).await;
    }
...
pub(crate) fn path_to_cstring(path: &Path) -> RS<CString> {
...
pub(crate) fn path_prefixes(path: &Path) -> Vec<PathBuf> {
