...
pub fn fn_char_order(v1: &DatInternal, v2: &DatInternal) -> Result<Ordering, ErrCompare> {
    let s1 = v1.to_any::<String>();
    let s2 = v2.to_any::<String>();
    Ok(s1.cmp(s2))
}
...
pub const FN_FIXED_LEN_STRING_COMPARE: FnCompare = FnCompare {
    order: fn_char_order,
    equal: fn_char_equal,
    hash: fn_char_hash,
};

pub const FN_FIXED_LEN_STRING_CONVERT: FnBase = FnBase {
    input: fn_char_in,
    output: fn_char_out,
    len: fn_char_len,
    recv: fn_char_recv,
