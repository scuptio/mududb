...
pub fn fn_f64_in(v: &DatPrintable, _p: &ParamObj) -> Result<DatInternal, ErrFnBase> {
    let r_i = v.str().parse::<f64>();
    let i = r_i.map_err(|e| ErrFnBase::ErrTypeConvert(e.to_string()))?;
    Ok(DatInternal::from_f64(i))
}

pub fn fn_f64_out(v: &DatInternal, _p: &ParamObj) -> Result<DatPrintable, ErrFnBase> {
    let i = v.to_f64();
...
pub fn fn_f64_send(v: &DatInternal, _p: &ParamObj) -> Result<DatBinary, ErrFnBase> {
    let i = v.to_f64();
...
pub fn fn_f64_send_to(v: &DatInternal, _p: &ParamObj, buf: &mut [u8]) -> Result<usize, ErrFnBase> {
    let i = v.to_f64();
...
pub fn fn_f64_recv(buf: &[u8], _p: &ParamObj) -> Result<DatInternal, ErrFnBase> {
    if size_of::<f64>() < buf.len() {
        return Err(ErrFnBase::ErrLowBufSpace(size_of::<f64>()));
    };
    let i = Endian::read_f64(buf);
    Ok(DatInternal::from_f64(i))
}

pub fn fn_f64_to_typed(v: &DatInternal, _p: &ParamObj) -> Result<DatTyped, ErrFnBase> {
    Ok(DatTyped::F64(v.to_f64()))
}

pub fn fn_f64_from_typed(v: &DatTyped, _p: &ParamObj) -> Result<DatInternal, ErrFnBase> {
    match v {
...
pub fn fn_f64_default(_p: &ParamObj) -> Result<DatInternal, ErrFnBase> {
    Ok(DatInternal::from_f64(f64::default()))
}
