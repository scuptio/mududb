...
pub fn deserialize_sized_from<D: DeserializeOwned + 'static>(
    deserialize: &[u8]
) -> RS<(D, usize)> {
    _deserialize_sized_from(deserialize)
}

pub fn deserialize_from_json<S: DeserializeOwned>(
    json: &str,
) -> RS<S> {
    _deserialize_from_json(json)
}

pub fn serialize_sized_to<S: Serialize>(
    serialize: &S,
    out_buf: &mut [u8],
) -> RS<(bool, u64)> {
...
pub fn serialize_sized_to_vec<S: Serialize>(
    serialize: &S
) -> RS<Vec<u8>> {
...
pub fn serialize_to_json<S: Serialize>(
    serialize: &S
) -> RS<String> {
    let string = serde_json::to_string_pretty(serialize)
        .map_err(|e| {
            m_error!(EC::EncodeErr, "serialize to json string error", e)
        })?;
    Ok(string)
}


fn _deserialize_from_json<S: DeserializeOwned>(
    json: &str,
) -> RS<S> {
    let s = serde_json::from_str::<S>(json)
        .map_err(|e| {
            m_error!(EC::DecodeErr, "deserialize json from string error", e)
        })?;
    Ok(s)
}


fn _serialize_sized_to_vec<S: Serialize>(
    serialize: &S
) -> RS<Vec<u8>> {
    ...
    if ok {
        ...
        if !ok2 {
            return Err(m_error!(EC::InsufficientBufferSpace, "insufficient buffer size to fill body"));
        }
    }
    Ok(vec)
}

fn _deserialize_sized_from<D: DeserializeOwned + 'static>(
    input: &[u8]
) -> RS<(D, usize)> {
    if input.len() < SIZE_LEN {
        return Err(m_error!(EC::InsufficientBufferSpace, "insufficient buffer size to fill length"));
    }
    ...
    let input_d: D = rmp_serde::decode::from_slice(&input[SIZE_LEN..SIZE_LEN + length as usize])
        .map_err(|e| {
            m_error!(EC::DecodeErr, "decode error", e)
        })?;

    Ok((input_d, length as _))
}

fn _serialize_sized_to<S: Serialize>(
    result: &S,
    out_buf: &mut [u8],
) -> RS<(bool, u64)> {
