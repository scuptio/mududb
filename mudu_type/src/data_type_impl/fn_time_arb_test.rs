#[cfg(test)]
mod tests {
    use crate::data_type_impl::fn_time_arb::{
        fn_time_arb_data_type_param, fn_time_arb_printable, fn_time_arb_val,
    };
    ...
    #[test]
    fn fn_time_arb_val_returns_time_data_value() {
        let mut u = Unstructured::new(BYTES);
        let dt = fn_time_arb_data_type_param(&mut u).unwrap();
        let mut u = Unstructured::new(BYTES);
        let value = fn_time_arb_val(&mut u, &dt).unwrap();
        assert_eq!(value.type_family().unwrap(), TypeFamily::Time);
    }
    ...
    fn fn_time_arb_dt_param_returns_time_type() {
        let mut u = Unstructured::new(BYTES);
        let dt = fn_time_arb_data_type_param(&mut u).unwrap();
        assert_eq!(dt.type_family(), TypeFamily::Time);
