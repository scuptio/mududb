use crate::data_binary::DataBinary;
use crate::data_json::DataJson;
use crate::data_textual::DataTextual;
use crate::data_type::DataType;
use crate::data_type_fn_convert::FnBase;
use crate::data_type_param_array::DataTypeParamArray;
use crate::data_value::DataValue;
use crate::type_error::{TyEC, TyErr};
use mudu::utils::bin_size::BinSize;
use mudu::utils::json::{JsonValue, from_json_str};
use mudu::utils::msg_pack::MsgPackValue;

pub fn fn_array_in(s: &str, data_type: &DataType) -> Result<DataValue, TyErr> {
    let json_value: JsonValue =
        from_json_str(s).map_err(|e| TyErr::new(TyEC::TypeConvertFailed, e.to_string()))?;
    let dat = fn_array_in_json(&json_value, data_type)?;
    Ok(dat)
}

pub fn fn_array_out(v: &DataValue, data_type: &DataType) -> Result<DataTextual, TyErr> {
    let json = fn_array_out_json(v, data_type)?;
    Ok(DataTextual::from(json.to_string()))
}

pub fn fn_array_in_json(json: &JsonValue, ty: &DataType) -> Result<DataValue, TyErr> {
    let param = array_param(ty);
    let opt_array = json.as_array();
    let array = match opt_array {
        Some(array) => array,
        None => {
            return Err(TyErr::new(
                TyEC::TypeConvertFailed,
                "expected a array in JSON".to_string(),
            ));
        }
    };
    let arr_elem_ty = param.data_type();
    let arr_elem_ty_id = arr_elem_ty.type_family();
    let mut value_array = Vec::with_capacity(array.len());
    for v in array.iter() {
        let data_value = arr_elem_ty_id.fn_input_json()(v, arr_elem_ty)?;
        value_array.push(data_value);
    }
    Ok(DataValue::from_array(value_array))
}

pub fn fn_array_out_json(v: &DataValue, dt: &DataType) -> Result<DataJson, TyErr> {
    let param = array_param(dt);
    let datum_array: &Vec<DataValue> = v.expect_array();
    let arr_elem_ty = param.data_type();
    let arr_elem_ty_id = arr_elem_ty.type_family();
    let mut vec_json_value = Vec::with_capacity(datum_array.len());
    for v in datum_array.iter() {
        let data_value = arr_elem_ty_id.fn_output_json()(v, arr_elem_ty)?;
        vec_json_value.push(data_value.into_json_value());
    }
    Ok(DataJson::from(JsonValue::Array(vec_json_value)))
}

pub fn fn_array_in_msgpack(msg_pack: &MsgPackValue, ty: &DataType) -> Result<DataValue, TyErr> {
    let param = array_param(ty);
    let opt_array = msg_pack.as_array();
    let array = match opt_array {
        Some(array) => array,
        None => {
            return Err(TyErr::new(
                TyEC::TypeConvertFailed,
                format!("expected a array in msgpack, got {:?}", opt_array),
            ));
        }
    };

    let mut vec = Vec::with_capacity(array.len());
    let ty_inner = param.data_type();
    for v in array {
        let data_value = ty_inner.type_family().fn_input_msg_pack()(v, ty_inner)?;
        vec.push(data_value);
    }
    Ok(DataValue::from_array(vec))
}

pub fn fn_array_out_msgpack(v: &DataValue, ty: &DataType) -> Result<MsgPackValue, TyErr> {
    let param = array_param(ty);
    let opt_array = v.as_array();
    let array = match opt_array {
        Some(array) => array,
        None => {
            return Err(TyErr::new(
                TyEC::TypeConvertFailed,
                format!("expected a array in value, got {:?}", opt_array),
            ));
        }
    };
    let mut vec = Vec::with_capacity(array.len());
    let ty_inner = param.data_type();
    for v in array.iter() {
        let msg_pack_value = ty_inner.type_family().fn_output_msg_pack()(v, ty_inner)?;
        vec.push(msg_pack_value);
    }
    Ok(MsgPackValue::Array(vec))
}

pub fn fn_type_output_len(_: &DataType) -> Result<Option<u32>, TyErr> {
    Ok(None)
}

fn header_size() -> usize {
    BinSize::size_of() * 2
}

pub fn fn_dat_output_len(data_value: &DataValue, data_type: &DataType) -> Result<u32, TyErr> {
    let param = array_param(data_type);
    let datum_array = data_value.expect_array();
    let mut size = header_size() as u32;
    let ty = param.data_type();
    let id = ty.type_family();
    for item_data_value in datum_array.iter() {
        let n = id.fn_send_data_len()(item_data_value, ty)?;
        size += n;
    }
    Ok(size)
}

fn handle_datum_array_recv(
    n: u32,
    binary: &[u8],
    dat_param: &DataType,
    vec: &mut Vec<DataValue>,
) -> Result<(), TyErr> {
    let mut offset = 0u32;
    for _i in 0..n {
        let (s, bytes) = dat_param.type_family().fn_recv()(&binary[offset as usize..], dat_param)?;
        vec.push(s);
        offset += bytes;
    }
    Ok(())
}

pub fn fn_array_send(data_value: &DataValue, data_type: &DataType) -> Result<DataBinary, TyErr> {
    let len = fn_dat_output_len(data_value, data_type)?;
    let mut vec = vec![0; len as usize];
    let _ = fn_array_send_to(data_value, data_type, &mut vec)?;
    Ok(DataBinary::from(vec))
}

pub fn fn_array_send_to(
    object: &DataValue,
    dat_param: &DataType,
    buf: &mut [u8],
) -> Result<u32, TyErr> {
    let param = array_param(dat_param);
    let datum_array: &Vec<DataValue> = object.expect_array();
    let hdr_size = header_size();
    let mut offset = hdr_size as u32;
    let ty = param.data_type();
    let id = ty.type_family();
    for item_data_value in datum_array.iter() {
        let s = id.fn_send_to()(item_data_value, ty, &mut buf[offset as usize..])?;
        offset += s;
    }
    let binary_bytes = BinSize::new(offset);
    binary_bytes.copy_to_slice(&mut buf[0..BinSize::size_of()]);
    let array_length = BinSize::new(datum_array.len() as u32);
    array_length.copy_to_slice(&mut buf[BinSize::size_of()..BinSize::size_of() * 2]);
    Ok(offset)
}

pub fn fn_array_recv(buf: &[u8], dat_param: &DataType) -> Result<(DataValue, u32), TyErr> {
    if buf.len() < header_size() {
        return Err(TyErr::new(
            TyEC::InsufficientSpace,
            "space insufficient error".to_string(),
        ));
    }

    let binary_bytes = BinSize::from_slice(&buf[0..BinSize::size_of()]).size();
    let array_length = BinSize::from_slice(&buf[BinSize::size_of()..BinSize::size_of() * 2]).size();
    if buf.len() < binary_bytes as usize {
        return Err(TyErr::new(
            TyEC::InsufficientSpace,
            "space insufficient error".to_string(),
        ));
    }
    let array_param = array_param(dat_param);
    let mut vec_object = Vec::with_capacity(array_length as usize);

    handle_datum_array_recv(
        array_length,
        &buf[header_size()..],
        array_param.data_type(),
        &mut vec_object,
    )?;
    Ok((DataValue::from_array(vec_object), binary_bytes))
}

pub fn fn_array_default(_: &DataType) -> Result<DataValue, TyErr> {
    Ok(DataValue::from_array(vec![]))
}

fn array_param(data_type: &DataType) -> &DataTypeParamArray {
    data_type.expect_array_param()
}

pub const FN_ARRAY_CONVERT: FnBase = FnBase {
    input_textual: fn_array_in,
    output_textual: fn_array_out,
    input_json: fn_array_in_json,
    output_json: fn_array_out_json,
    input_msg_pack: fn_array_in_msgpack,
    output_msg_pack: fn_array_out_msgpack,
    type_len: fn_type_output_len,
    data_len: fn_dat_output_len,
    receive: fn_array_recv,
    send: fn_array_send,
    send_to: fn_array_send_to,
    default: fn_array_default,
};
