...
fn normalize_numeric_for_type(value: &Numeric, dt: &DatType) -> Result<Numeric, TyErr> {
...
fn numeric_scale(dt: &DatType) -> u8 {
    dt.as_numeric_param()
        .map(|param| param.scale())
        .unwrap_or(0)
}

fn scaled_i128_from_numeric(value: &Numeric, dt: &DatType) -> Result<i128, TyErr> {
...
fn numeric_from_scaled_i128(scaled: i128, dt: &DatType) -> Result<Numeric, TyErr> {
    let scale = numeric_scale(dt) as usize;
    if scale == 0 {
...
fn fn_numeric_in_textual(v: &str, dt: &DatType) -> Result<DatValue, TyErr> {
    let json = from_json_str::<JsonValue>(v)
        .map_err(|e| TyErr::new(TyEC::TypeConvertFailed, e.to_string()))?;
    fn_numeric_in_json(&json, dt)
}

fn fn_numeric_out_textual(v: &DatValue, dt: &DatType) -> Result<DatTextual, TyErr> {
    let json = fn_numeric_out_json(v, dt)?;
    Ok(DatTextual::from(json.to_string()))
}

fn fn_numeric_in_json(v: &JsonValue, dt: &DatType) -> Result<DatValue, TyErr> {
    Ok(DatValue::from_numeric(normalize_numeric_for_type(
        &parse_numeric_json(v)?,
        dt,
    )?))
}

fn fn_numeric_out_json(v: &DatValue, dt: &DatType) -> Result<DatJson, TyErr> {
    Ok(DatJson::from(JsonValue::String(
        normalize_numeric_for_type(v.expect_numeric(), dt)?.to_plain_string(),
    )))
}

fn fn_numeric_in_msgpack(msg_pack: &MsgPackValue, dt: &DatType) -> Result<DatValue, TyErr> {
    if let Some(s) = msg_pack.as_str() {
        return Ok(DatValue::from_numeric(normalize_numeric_for_type(
            &parse_numeric_str(s)?,
            dt,
        )?));
    }
...
fn fn_numeric_out_msgpack(v: &DatValue, dt: &DatType) -> Result<MsgPackValue, TyErr> {
    Ok(MsgPackValue::String(MsgPackUtf8String::from(
        normalize_numeric_for_type(v.expect_numeric(), dt)?.to_plain_string(),
    )))
}

fn fn_numeric_len(_: &DatType) -> Result<Option<u32>, TyErr> {
    Ok(Some(size_of::<i128>() as u32))
}

fn fn_numeric_dat_output_len(val: &DatValue, ty: &DatType) -> Result<u32, TyErr> {
    let _ = scaled_i128_from_numeric(val.expect_numeric(), ty)?;
    Ok(size_of::<i128>() as u32)
}

fn fn_numeric_send(v: &DatValue, dt: &DatType) -> Result<DatBinary, TyErr> {
...
fn fn_numeric_send_to(v: &DatValue, dt: &DatType, buf: &mut [u8]) -> Result<u32, TyErr> {
...
fn fn_numeric_recv(buf: &[u8], dt: &DatType) -> Result<(DatValue, u32), TyErr> {
    if buf.len() < size_of::<i128>() {
        return Err(TyErr::new(
            TyEC::InsufficientSpace,
            "insufficient space".to_string(),
    ...
    Ok((
        DatValue::from_numeric(normalize_numeric_for_type(&numeric, dt)?),
        size_of::<i128>() as u32,
    ))
}

fn fn_numeric_default(dt: &DatType) -> Result<DatValue, TyErr> {
    Ok(DatValue::from_numeric(normalize_numeric_for_type(
        &Numeric::zero(),
        dt,
    )?))
...
fn fn_numeric_order(v1: &DatValue, v2: &DatValue) -> Result<Ordering, ErrCompare> {
    Ok(v1.expect_numeric().cmp(v2.expect_numeric()))
}

fn fn_numeric_equal(v1: &DatValue, v2: &DatValue) -> Result<bool, ErrCompare> {
    Ok(v1.expect_numeric() == v2.expect_numeric())
}

fn fn_numeric_hash(v: &DatValue, hasher: &mut dyn Hasher) -> Result<(), ErrCompare> {
...
mod tests {
    ...
    fn numeric_binary_roundtrip_uses_scaled_i128() {
        let ty = DatType::from_numeric(DTPNumeric::new(18, 4));
        let value = DatValue::from_numeric(Numeric::parse("12.3400").unwrap());
    ...
    fn numeric_binary_length_is_fixed() {
        let ty = DatType::from_numeric(DTPNumeric::new(18, 2));
    ...
    fn numeric_binary_roundtrip_preserves_negative_scaled_values() {
        let ty = DatType::from_numeric(DTPNumeric::new(18, 4));
        let value = DatValue::from_numeric(Numeric::parse("-0.0100").unwrap());
        let binary = fn_numeric_send(&value, &ty).unwrap();
    ...
    fn numeric_binary_send_rounds_half_even_to_declared_scale() {
        let ty = DatType::from_numeric(DTPNumeric::new(5, 2));
        let lower = DatValue::from_numeric(Numeric::parse("1.235").unwrap());
        let upper = DatValue::from_numeric(Numeric::parse("1.245").unwrap());
    ...
    fn numeric_binary_send_rejects_precision_overflow_after_rounding() {
        let ty = DatType::from_numeric(DTPNumeric::new(4, 2));
        let value = DatValue::from_numeric(Numeric::parse("999.995").unwrap());
    ...
    fn numeric_binary_send_to_rejects_short_buffer() {
        let ty = DatType::from_numeric(DTPNumeric::new(18, 2));
        let value = DatValue::from_numeric(Numeric::parse("7.50").unwrap());
    ...
    fn numeric_binary_encoding_sorts_negative_before_positive() {
        let ty = DatType::from_numeric(DTPNumeric::new(9, 4));
        let negative = DatValue::from_numeric(Numeric::parse("-0.0100").unwrap());
        let positive = DatValue::from_numeric(Numeric::parse("12.3400").unwrap());
