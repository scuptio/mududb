...
fn timestamp_type(precision: u8) -> DatType {
    DatType::from_timestamp(DTPTimestamp::new(precision))
}
...
fn timestamp_len_and_output_len_are_eight_bytes() {
    let ty = timestamp_type(6);
    let value = DatValue::from_timestamp(TimestampValue::from_epoch_micros(0));
...
fn timestamp_order_equal_and_hash() {
    let a = DatValue::from_timestamp(TimestampValue::from_epoch_micros(100));
    let b = DatValue::from_timestamp(TimestampValue::from_epoch_micros(200));
    let a2 = DatValue::from_timestamp(TimestampValue::from_epoch_micros(100));
