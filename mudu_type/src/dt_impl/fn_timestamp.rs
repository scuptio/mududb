...
fn parse_timestamp_str(value: &str, dt: &DatType) -> Result<TimestampValue, TyErr> {
...
fn fn_timestamp_in_textual(v: &str, dt: &DatType) -> Result<DatValue, TyErr> {
    let json = from_json_str::<JsonValue>(v)
        .map_err(|e| TyErr::new(TyEC::TypeConvertFailed, e.to_string()))?;
    fn_timestamp_in_json(&json, dt)
}

fn fn_timestamp_out_textual(v: &DatValue, dt: &DatType) -> Result<DatTextual, TyErr> {
    let json = fn_timestamp_out_json(v, dt)?;
    Ok(DatTextual::from(json.to_string()))
}

fn fn_timestamp_in_json(v: &JsonValue, dt: &DatType) -> Result<DatValue, TyErr> {
    Ok(DatValue::from_timestamp(parse_timestamp_str(
        parse_temporal_json_string(v, "timestamp")?.as_str(),
        dt,
    )?))
}

fn fn_timestamp_out_json(v: &DatValue, dt: &DatType) -> Result<DatJson, TyErr> {
...
fn fn_timestamp_in_msgpack(msg_pack: &MsgPackValue, dt: &DatType) -> Result<DatValue, TyErr> {
    ...
    Ok(DatValue::from_timestamp(parse_timestamp_str(s, dt)?))
}

fn fn_timestamp_out_msgpack(v: &DatValue, dt: &DatType) -> Result<MsgPackValue, TyErr> {
...
fn fn_timestamp_len(_: &DatType) -> Result<Option<u32>, TyErr> {
    Ok(Some(size_of::<i64>() as u32))
}

fn fn_timestamp_dat_output_len(_: &DatValue, _: &DatType) -> Result<u32, TyErr> {
    Ok(size_of::<i64>() as u32)
}

fn fn_timestamp_send(v: &DatValue, _: &DatType) -> Result<DatBinary, TyErr> {
...
fn fn_timestamp_send_to(v: &DatValue, _: &DatType, buf: &mut [u8]) -> Result<u32, TyErr> {
...
fn fn_timestamp_recv(buf: &[u8], _: &DatType) -> Result<(DatValue, u32), TyErr> {
    if buf.len() < size_of::<i64>() {
        return Err(TyErr::new(
            TyEC::InsufficientSpace,
            "insufficient space".to_string(),
    ...
    Ok((
        DatValue::from_timestamp(TimestampValue::from_epoch_micros(decode_sortable_i64(
            Endian::read_u64(buf),
        ))),
        size_of::<i64>() as u32,
    ))
...
fn fn_timestamp_default(_: &DatType) -> Result<DatValue, TyErr> {
    Ok(DatValue::from_timestamp(TimestampValue::from_epoch_micros(
        0,
    )))
}

fn fn_timestamp_order(v1: &DatValue, v2: &DatValue) -> Result<Ordering, ErrCompare> {
    Ok(v1.expect_timestamp().cmp(v2.expect_timestamp()))
}

fn fn_timestamp_equal(v1: &DatValue, v2: &DatValue) -> Result<bool, ErrCompare> {
    Ok(v1.expect_timestamp() == v2.expect_timestamp())
}

fn fn_timestamp_hash(v: &DatValue, hasher: &mut dyn Hasher) -> Result<(), ErrCompare> {
...
mod tests {
    ...
    fn timestamp_binary_roundtrip_respects_precision() {
        let ty = DatType::from_timestamp(DTPTimestamp::new(4));
        let value =
            DatValue::from_timestamp(TimestampValue::parse("2026-05-20 14:30:45.123456").unwrap());
    ...
    fn timestamp_json_and_msgpack_io_respect_precision() {
        let ty = DatType::from_timestamp(DTPTimestamp::new(3));
    ...
    fn timestamp_default_is_unix_epoch() {
        let ty = DatType::from_timestamp(DTPTimestamp::new(6));
