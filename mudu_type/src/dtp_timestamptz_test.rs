...
mod tests {
    ...
    fn timestamptz_validate_rejects_excessive_precision() {
        assert!(
            DTPTimestampTz::new(TEMPORAL_MAX_PRECISION + 1)
                .validate()
                .is_err()
        );
    }
    ...
    fn timestamptz_json_roundtrip() {
        let t = DTPTimestampTz::new(2);
        let json = t.se_to_json().unwrap();
        let mut restored = DTPTimestampTz::default();
    ...
    fn timestamptz_compare_orders_by_precision() {
        let a = DTPTimestampTz::new(1);
        let b = DTPTimestampTz::new(3);
