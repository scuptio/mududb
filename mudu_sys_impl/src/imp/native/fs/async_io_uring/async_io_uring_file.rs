...
use crate::imp::fs::async_io_uring::io_uring_file::IoUringFile;
use async_trait::async_trait;
use mudu::common::result::RS;
use std::os::fd::RawFd;
use std::path::Path;

pub struct AsyncIoUringFile {
    inner: IoUringFile,
}

impl AsyncIoUringFile {
    pub(crate) async fn open(path: impl AsRef<Path>, options: FileOptions) -> RS<Self> {
        Ok(Self {
            inner: IoUringFile::open(path, options).await?,
        })
    }
}

#[async_trait]
impl AsyncFile for AsyncIoUringFile {
