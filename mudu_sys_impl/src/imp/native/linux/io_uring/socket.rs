...
use crate::imp::native::linux::io_uring::worker_ring::{
    with_current_ring, WorkerLocalRing, WorkerRingOp,
};
...
pub async fn connect(sock: &IoSocket, addr: SocketAddr) -> RS<()> {
    let raw_addr = socket_addr_to_raw(addr)?;
    SocketConnectFuture::new(sock.fd, raw_addr).await
}
...
impl SocketConnectFuture {
    fn new(fd: RawFd, addr: crate::imp::native::linux::io_uring::iouring::SockAddrBuf) -> Self {
        Self {
            fd,
            addr: Some(addr),
            state: SocketFutureState::Init,
            ...
            SocketFutureState::Init => {
                ...
                let Some(addr) = self.addr.take() else {
                    ...
                    return Poll::Ready(Err(mudu_error!(
                        ErrorCode::Internal,
                        "SocketConnectFuture polled after completion"
                    )));
                };
                if let Err(err) = with_current_ring(|ring| {
        ...
        match &self.state {
            SocketFutureState::Init => {
                ...
                let Some(data) = self.data.take() else {
                    ...
                    return Poll::Ready(Err(mudu_error!(
                        ErrorCode::Internal,
                        "SocketSendFuture polled after completion"
                    )));
                };
                if let Err(err) = with_current_ring(|ring| {
...
fn socket_addr_to_raw(
    addr: SocketAddr,
) -> RS<crate::imp::native::linux::io_uring::iouring::SockAddrBuf> {
    crate::imp::io::net::socket_addr_to_storage(addr).map_err(|e| {
        mudu_error!(
            ErrorCode::Network,
            "socket addr to storage conversion failed",
            e
        )
    })
}
...
pub fn raw_to_socket_addr(
    ...
    crate::imp::io::net::sockaddr_to_socket_addr(addr)
}
...
mod tests {
    ...
    async fn accept_recv_send_shutdown_and_close_enqueue_requests() {
        ...
        let recv_task = spawn_task_detached("test", async move {
            ...
            Ok::<_, mudu::error::MuduError>((read, buf))
        })
        .unwrap();
        yield_now().await;
        match ring.take_pending().unwrap().unwrap().1 {
