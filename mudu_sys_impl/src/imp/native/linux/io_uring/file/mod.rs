...
use crate::io::user_io::{OpState, poll_op};
#[cfg(target_os = "linux")]
...
pub(crate) use file_io_future::{FileIoFuture, poll_file_io_future};
...
pub async fn metadata_len<P: AsRef<Path>>(path: P) -> RS<u64> {
    crate::io::fs::metadata_len(path.as_ref())
        .await
...
#[cfg(target_os = "linux")]
fn assert_worker_ring() {
    if !crate::io::worker_ring::has_current_worker_ring() {
        panic!("IoFile operation requires a current worker ring");
    }
}
...
mod tests {
    ...
    #[tokio::test(flavor = "current_thread")]
    #[should_panic(expected = "IoFile operation requires a current worker ring")]
    async fn open_without_current_ring_panics() {
        unset_current_worker_ring();
