...
pub struct Sys {
    default_provider: SyncUnsafeCell<Arc<dyn AsyncIoProvider>>,
    tokio_provider: SyncUnsafeCell<Arc<dyn AsyncIoProvider>>,
}
...
impl Sys {
    pub fn new() -> Self {
        Self {
            default_provider: SyncUnsafeCell::new(create_io_provider(ProviderType::Tokio)),
            tokio_provider: SyncUnsafeCell::new(create_io_provider(ProviderType::Tokio)),
        }
    }

    pub fn setup(&self, provider_type: ProviderType) {
        unsafe {
            *self.default_provider.get() = create_io_provider(provider_type);
        }
    }

    pub fn provider(&self) -> &dyn AsyncIoProvider {
        unsafe { (*self.default_provider.get()).as_ref() }
    }

    pub fn provider_arc(&self) -> Arc<dyn AsyncIoProvider> {
        unsafe { (*self.default_provider.get()).clone() }
    }

    pub fn tokio_provider(&self) -> &dyn AsyncIoProvider {
        unsafe { (*self.tokio_provider.get()).as_ref() }
    }
    ...
    pub fn io_uring(entries: u32) -> RS<super::io_uring::IoUring> {
        super::io_uring::IoUring::new(entries)
            .map_err(|e| m_error!(EC::IOErr, format!("create io_uring error: {e}")))
    }
