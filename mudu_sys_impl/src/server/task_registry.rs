...
use crate::task::async_::{PollTaskIdGuard, try_this_task_id};
...
mod tests {
    ...
    fn external_wake_notifies_worker_eventfd_and_repolls_task() {
        let fd = crate::sync::blocking::eventfd().unwrap();
        ...
        tx.send(()).unwrap();
        assert_eq!(crate::sync::blocking::read_eventfd(fd).unwrap(), 1);
        ...
        crate::sync::blocking::close_fd(fd).unwrap();
    }
}
