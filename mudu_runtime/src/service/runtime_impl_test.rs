...
mod tests {
    ...
    fn temp_path(prefix: &str) -> PathBuf {
        mudu_sys::env_var::temp_dir().join(format!("{}_{}", prefix, mudu_sys::random::uuid_v4()))
    }

    // These tests create a full runtime service backed by Wasmtime and libsql,
    // both of which call foreign functions that Miri does not support. They are
    // ignored under Miri and run only on native builds.
    #[test]
    ...
    fn create_runtime_service_injects_tokio_async_runtime() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            ...
            assert!(matches!(async_runtime.mode(), AsyncMode::Tokio));
