...
use crate::procedure::wasi_context::{WasiContext, build_wasi_p1_context};
use anyhow::Context;
...
impl ProcedureInvoke1 {
    pub fn call(
        ...
        let context = build_wasi_p1_context(worker_local);
        let this: Self = Self::new(
            context,
            procedure.instance().as_p1_instance_pre(),
            name,
            proc_opt,
        )?;
        this.invoke(param)
...
impl ProcedureInvokeInner {
    ...
    fn process_invoke_result(
        &mut self,
        invoke_param: &InvokeParam,
        r: wasmtime::Result<i32>,
    ) -> RS<ProcedureResult> {
        match r {
            Ok(code) => {
                if code == 0 {
