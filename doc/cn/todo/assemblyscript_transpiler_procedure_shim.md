...
> **历史设计文档 / 部分已废弃**
>
> 本文档记录 AssemblyScript procedure shim 的**早期设计思路**。实际落地实现与本文档的签名约定存在差异：
>
> - transpiler 不再要求业务过程使用 `ValueList -> ValueList` 的通用签名；
> - 当前实现要求过程首参为 `Oid`，后续参数为具体标量，返回值为标量或 `Result<T>`；
> - 生成的 `adapter_P` 仍保持 `ValueList -> MuduResult<ValueList>` 形态，但参数解包与返回值装箱由 transpiler 自动生成。
>
> 具体实现参见：
> - `mudu_transpiler/src/assemblyscript/parser.rs`
> - `mudu_transpiler/src/assemblyscript/tests.rs`
> - `mudu_transpiler/readme.md`
>
> 以下内容保留作为设计背景参考，但**不应作为编写新 procedure 的依据**。
...
3. **支持注释导语标记 procedure**  
   ...
   transpiler 通过该标记发现 AssemblyScript procedure。实际实现中，首参必须是 `Oid`，后续为标量参数，返回标量或 `Result<T>`（参见上方【历史设计文档】说明）。
...
3. ~~**不要求第一阶段支持泛型或复杂静态类型映射**~~  
   早期设计考虑只支持 `ValueList -> ValueList` 的通用调用模式；实际实现已转向强类型标量参数，由 transpiler 自动生成 `ValueList` 解包/装箱代码。
...
```ts
/**mudu-proc*/
export function P(id: Oid, arg1: i64, arg2: string): Result<i64> {
  ...
}
```

实际 parser 要求首参为 `Oid`，后续参数为标量，返回标量或 `Result<T>`。
...
则生成对应 adapter（由 transpiler 自动完成 `ValueList` 解包与返回值装箱）：

```ts
export function adapter_P(id: Oid, values: ValueList): MuduResult<ValueList> {
  const arg1 = values.value(0).asInt64();
  const arg2 = values.value(1).asText();
  const result = P(id, arg1, arg2);
  if (result.isErr) {
    return __muduProcedureResultErr(result.unwrapErr(), "P");
  }
  const returnValues = new ValueList();
  returnValues.bind(0, MuduValue.int64(result.unwrap()));
