...
fn custom_types() -> UniTypeDesc {
    ...
    desc.types
        .insert("MyI32".to_string(), UniDatType::Scalar(UniScalar::I32));
    desc
}
...
#[test]
fn to_dat_type_maps_primitives_and_customs() -> Result<(), Box<dyn Error>> {
...
#[test]
fn to_dat_type_rejects_unknown_types() -> Result<(), Box<dyn Error>> {
    let custom = custom_types();
...
#[test]
fn to_dat_type_handles_vec_and_vec_u8() -> Result<(), Box<dyn Error>> {
    let custom = custom_types();
    ...
    assert_eq!(
        vec_u8.to_dat_type(&custom)?.dat_type_id(),
        DatTypeID::Binary
    );
    ...
    let dt = vec_i32.to_dat_type(&custom)?;
...
#[test]
fn tuple_and_custom_to_dat_type_are_rejected() -> Result<(), Box<dyn Error>> {
    let custom = custom_types();
