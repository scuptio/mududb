...
mod tests {
    ...
    fn scalar() -> UniDataValue {
        UniDataValue::from_scalar(UniScalarValue::Bool(true))
    }

    fn array() -> UniDataValue {
        UniDataValue::from_array(vec![scalar()])
    }

    fn record() -> UniDataValue {
        UniDataValue::from_record(vec![UniDataValueField {
            field_name: "flag".to_string(),
            field_value: scalar(),
        }])
    }

    fn binary() -> UniDataValue {
        UniDataValue::from_binary(vec![1, 2, 3])
    }
    ...
    fn default_is_scalar_false() {
        assert!(matches!(
            ...
            UniDataValue::Scalar(UniScalarValue::Bool(false))
        ));
    }
    ...
    fn serialize_roundtrips_through_json() {
        ...
        for value in values {
            ...
            let decoded: UniDataValue = serde_json::from_str(&json).unwrap();
    ...
    fn deserialize_rejects_empty_sequence() {
        let json = "[]";
        let result: Result<UniDataValue, _> = serde_json::from_str(json);
    ...
    fn deserialize_rejects_missing_scalar_value() {
        let result: Result<UniDataValue, _> = serde_json::from_str("[0]");
        assert!(result.is_err());
    }

    #[test]
    fn deserialize_rejects_missing_array_value() {
        let result: Result<UniDataValue, _> = serde_json::from_str("[1]");
        assert!(result.is_err());
    }

    #[test]
    fn deserialize_rejects_missing_record_value() {
        let result: Result<UniDataValue, _> = serde_json::from_str("[2]");
        assert!(result.is_err());
    }

    #[test]
    fn deserialize_rejects_missing_binary_value() {
        let result: Result<UniDataValue, _> = serde_json::from_str("[3]");
        assert!(result.is_err());
    }

    fn assert_same_value(actual: &UniDataValue, expected: &UniDataValue) {
        match (actual, expected) {
            (UniDataValue::Scalar(a), UniDataValue::Scalar(b)) => {
                assert!(
                    matches!((a, b), (UniScalarValue::Bool(a), UniScalarValue::Bool(b)) if a == b)
                );
            }
            (UniDataValue::Array(a), UniDataValue::Array(b)) => {
                assert_eq!(a.len(), b.len());
                for (x, y) in a.iter().zip(b.iter()) {
                    assert_same_value(x, y);
                }
            }
            (UniDataValue::Record(a), UniDataValue::Record(b)) => {
                assert_eq!(a.len(), b.len());
                for (x, y) in a.iter().zip(b.iter()) {
                    assert_eq!(x.field_name, y.field_name);
                    assert_same_value(&x.field_value, &y.field_value);
                }
            }
            (UniDataValue::Binary(a), UniDataValue::Binary(b)) => assert_eq!(a, b),
