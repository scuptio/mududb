...
module.exports = grammar({
  ...
  rules: {
    ...
    variant_items: ($) =>
      seq(field('comment', repeat($.special_comment)), 'variant', field('variant_name', $.id), $._variant_body),
    _variant_body: ($) =>
      seq('{', $._variant_cases, '}'),

    _variant_cases: ($) => commaSeparatedList(field('variant_case', $.variant_case)),
    variant_case: ($) =>
      choice(
        seq(field('comment', repeat($.special_comment)), field('variant_case_name', $.id)),
        seq(field('comment', repeat($.special_comment)), field('variant_case_name', $.id), '(', field('variant_case_type', $.ty), ')'),
      ),

    enum_items: ($) => seq(field('comment', repeat($.special_comment)), 'enum', field('enum_name', $.id), $._enum_body),
    _enum_body: ($) => seq('{', $._enum_cases, '}'),
    _enum_cases: ($) => commaSeparatedList(field('enum_case', $.enum_case)),
    enum_case: ($) => seq(field('comment', repeat($.special_comment)), field('enum_case_name', $.id)),
    resource_item: ($) =>
      seq(
        'resource',
        field('name', $.id),
    ...
    bool:($) => 'bool',
    string: ($) => 'string',
    tuple: ($) => seq('tuple', '<', field('tuple_list', $.tuple_list), '>'),
    ...
    list: ($) => seq('list',
      '<',
        field("list_inner_type", $.ty),
      optional(field('size', seq(',', $.uint))),
      '>',
    ),

      option: ($) => seq('option', '<', field('option_inner_type', $.ty), '>'),
    ...
    result: ($) =>
      seq(
        ...
        optional(
          choice(
              seq('<', field('result_ok_type', $.ty), ',', field('result_err_type', $.ty), '>'),
              seq('<', '_', ',', field('result_err_type', $.ty), '>'),
              seq('<', field('result_err_type', $.ty), '>'),
          ),
        ),
      ),
    ...
    box: ($) => seq('box', '<', field('box_inner', $.ty), '>'),
    ...
    special_comment: $ => choice(
        $.special_line_comment,
        $.special_block_comment,
    ),

    line_comment: $ => prec(-1, seq(
