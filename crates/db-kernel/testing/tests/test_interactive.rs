...
use mudu_client::client::json_client::JsonClient;
...
fn run_interactive_mcli_shell_test(server_mode: ServerMode) -> RS<()> {
    ...
    // Length-changing multi-column VARCHAR update: name grows, city shrinks.
    assert!(shell_output.contains("\"Evelyn\""));
    assert!(shell_output.contains("\"Palo Alto\""));
    assert!(shell_output.contains("\"Rome\""));
    // `SET city = NULL` stores SQL NULL (reads back as JSON null), and the
    // column accepts a value again afterwards.
    assert!(shell_output.contains("[\"Evelyn\",null]"));
    assert!(shell_output.contains("\"Oslo\""));
    drop(server);
    Ok(())
}
...
fn crud_script() -> &'static str {
    concat!(
        ...
        "CREATE TABLE t_crud(id INT PRIMARY KEY, name TEXT, city TEXT);\n",
        "INSERT INTO t_crud(id, name, city) VALUES (1, 'Eve', 'Palo Alto');\n",
        "SELECT name, city FROM t_crud WHERE id = 1;\n",
        "UPDATE t_crud SET name = 'Eva' WHERE id = 1;\n",
        "SELECT name, city FROM t_crud WHERE id = 1;\n",
        "UPDATE t_crud SET name = 'Evelyn', city = 'Rome' WHERE id = 1;\n",
        "SELECT name, city FROM t_crud WHERE id = 1;\n",
        "UPDATE t_crud SET city = NULL WHERE id = 1;\n",
        "SELECT name, city FROM t_crud WHERE id = 1;\n",
        "UPDATE t_crud SET city = 'Oslo' WHERE id = 1;\n",
        "SELECT name, city FROM t_crud WHERE id = 1;\n",
