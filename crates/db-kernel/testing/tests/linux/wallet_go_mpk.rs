...
static WALLET_GO_MPK_TEST_LOCK: LazyLock<SMutex<()>> = LazyLock::new(|| SMutex::new(()));

// End-to-end check that the TinyGo guest packaged as `wallet-go.mpk` speaks
// the MSSP v1 ABI against the real runtime: the Go component, which imports
// `mududb:api/system` directly and frames MSSP through the in-repo Go
// binding (`types` + `codec`), is installed into a full mudud backend and
// its procedures are invoked over HTTP. Correct query/command results prove
// the guest's syscalls were framed through MSSP correctly.
//
// The test skips (passes) when wallet-go.mpk has not been built, so machines
// without the TinyGo toolchain stay green.
#[cfg_attr(miri, ignore)]
#[test]
fn wallet_go_mpk_http_end_to_end_tokio() -> RS<()> {
    let _guard = WALLET_GO_MPK_TEST_LOCK
        .lock()
        .expect("wallet-go mpk test lock poisoned");
    let mpk_path = wallet_go_mpk_path();
    if !mpk_path.exists() {
        eprintln!(
            "skip wallet-go HTTP test: {} not built (cargo make package in example/wallet-go)",
            mpk_path.display()
        );
        return Ok(());
    }
    let Some(ctx) = TestContext::new(ServerMode::Tokio)? else {
        eprintln!("skip wallet-go HTTP test: local TCP/HTTP bind is not permitted");
        return Ok(());
    };
    let server = ctx.start_server()?;

    let mpk_binary = read(&mpk_path).expect("read wallet-go.mpk");
    ...
    assert_eq!(apps, json!(["wallet-go"]));

    let procedures = ctx.get_json("/mudu/app/list/wallet-go")?;
    let procedure_list = procedures["procedures"].as_array().expect("procedure list");
    assert!(procedure_list.contains(&json!("wallet_go/create_user")));
    assert!(procedure_list.contains(&json!("wallet_go/deposit")));
    assert!(procedure_list.contains(&json!("wallet_go/withdraw")));
    assert!(procedure_list.contains(&json!("wallet_go/transfer_funds")));
    assert!(procedure_list.contains(&json!("wallet_go/balance")));
    assert!(procedure_list.contains(&json!("wallet_go/update_profile")));
    ...
    // An overdrawing withdrawal must raise inside the Go guest and surface
    // as an invoke error through MSSP error propagation.
    let overdraw = ctx.invoke("withdraw", json!({ "user_id": 1, "amount": 1_000_000 }));
    assert!(overdraw.is_err());

    assert!(ctx.user_exists("wallet-go", 3)?);
    assert_eq!(ctx.user_name("wallet-go", 3)?, "Carol");
    assert_eq!(ctx.wallet_balance("wallet-go", 1)?, 9750);
    assert_eq!(ctx.wallet_balance("wallet-go", 2)?, 10000);
    ...
    // DataTypeParamRecord into the positional record envelope, the Go guest
    // decodes it through the binding record bridge plus the mgen-generated
    // codec, stores it, and returns the stored record. `vip` is a bool in
    // WIT but crosses the JSON/desc surface as i32 (0/1) — the host has no
    ...
    let profile = ctx.profile_row("wallet-go", 1)?;
    assert_eq!(
        profile,
        Some((
    ...
    assert_eq!(ctx.profile_tags("wallet-go", 1)?, vec!["c".to_string()]);
...
// and the tag list is the array case `[1, items]`.
fn assert_return_profile(response: &Value, expected: &Value) {
    ...
    let expected_record = json!([
        ...
        [
            ...
            to_field(i32_datum(expected["level"].as_i64().expect("level"))),
...
impl TestContext {
    fn new(server_mode: ServerMode) -> RS<Option<Self>> {
        ...
        let base_dir = temp_dir("mududb-testing-wallet-go");
    ...
    fn invoke(&self, proc_name: &str, params: Value) -> RS<Value> {
        self.post_json(
            &format!("/mudu/app/invoke/wallet-go/wallet_go/{proc_name}"),
            params,
        )
    }
