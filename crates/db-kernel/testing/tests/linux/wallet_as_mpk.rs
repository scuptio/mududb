...
static WALLET_AS_MPK_TEST_LOCK: LazyLock<SMutex<()>> = LazyLock::new(|| SMutex::new(()));

// End-to-end check that the AssemblyScript guest packaged as `wallet-as.mpk`
// speaks the MSSP v1 ABI against the real runtime: the AssemblyScript
// component, which imports `mududb:api/system` directly and frames MSSP
// itself, is installed into a full mudud backend and its procedures are
// invoked over HTTP. Correct query/command results prove the guest's
// syscalls were framed through MSSP correctly.
//
// This integration test starts a full mudud backend server and exercises the
// wallet-as MPK via HTTP/TCP, which performs foreign-function calls and
// network I/O that Miri cannot emulate. It is ignored under Miri and runs
// only on native Linux builds.
#[cfg_attr(miri, ignore)]
#[test]
fn wallet_as_mpk_http_end_to_end_tokio() -> RS<()> {
    let _guard = WALLET_AS_MPK_TEST_LOCK
        .lock()
        .expect("wallet-as mpk test lock poisoned");
    let Some(ctx) = TestContext::new(ServerMode::Tokio)? else {
        eprintln!("skip wallet-as HTTP test: local TCP/HTTP bind is not permitted");
        return Ok(());
    };
    let server = ctx.start_server()?;

    let mpk_binary = read(ctx.wallet_as_mpk_path()).expect("read wallet-as.mpk");
    ...
    assert_eq!(apps, json!(["wallet-as"]));

    let procedures = ctx.get_json("/mudu/app/list/wallet-as")?;
    let procedure_list = procedures["procedures"].as_array().expect("procedure list");
    assert!(procedure_list.contains(&json!("wallet_as/create_user")));
    assert!(procedure_list.contains(&json!("wallet_as/deposit")));
    assert!(procedure_list.contains(&json!("wallet_as/withdraw")));
    assert!(procedure_list.contains(&json!("wallet_as/transfer_funds")));
    assert!(procedure_list.contains(&json!("wallet_as/balance")));

    let detail = ctx.get_json("/mudu/app/list/wallet-as/wallet_as/create_user")?;
    ...
    // An overdrawing withdrawal must trap inside the AssemblyScript guest and
    ...
    assert!(ctx.user_exists("wallet-as", 3)?);
    assert_eq!(ctx.user_name("wallet-as", 3)?, "Carol");
    assert_eq!(ctx.wallet_balance("wallet-as", 1)?, 9750);
    assert_eq!(ctx.wallet_balance("wallet-as", 2)?, 10000);
...
impl TestContext {
    fn new(server_mode: ServerMode) -> RS<Option<Self>> {
        ...
        let base_dir = temp_dir("mududb-testing-wallet-as");
    ...
    fn wallet_as_mpk_path(&self) -> PathBuf {
        workspace_root()
            .join("testing")
            .join("mpk")
            .join("wallet-as.mpk")
    ...
    fn invoke(&self, proc_name: &str, params: Value) -> RS<Value> {
        self.post_json(
            &format!("/mudu/app/invoke/wallet-as/wallet_as/{proc_name}"),
            params,
        )
    }
