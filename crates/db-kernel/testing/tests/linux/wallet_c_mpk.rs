...
static WALLET_C_MPK_TEST_LOCK: LazyLock<SMutex<()>> = LazyLock::new(|| SMutex::new(()));

// End-to-end check that the freestanding C guest packaged as `wallet-c.mpk`
// speaks the MSSP v1 ABI against the real runtime: the C component, which
// imports `mududb:api/system` directly and frames MSSP through the in-repo
// C binding (`mududb/types` + `mududb/codec`), is installed into a full
// mudud backend and its procedures are invoked over HTTP. Correct
// query/command results prove the guest's syscalls were framed through MSSP
// correctly.
//
// The test skips (passes) when wallet-c.mpk has not been built, so machines
// without a wasm32 clang toolchain stay green.
#[cfg_attr(miri, ignore)]
#[test]
fn wallet_c_mpk_http_end_to_end_tokio() -> RS<()> {
    let _guard = WALLET_C_MPK_TEST_LOCK
        .lock()
        .expect("wallet-c mpk test lock poisoned");
    let mpk_path = wallet_c_mpk_path();
    if !mpk_path.exists() {
        eprintln!(
            "skip wallet-c HTTP test: {} not built (cargo make package in example/wallet-c)",
            mpk_path.display()
        );
        return Ok(());
    }
    let Some(ctx) = TestContext::new(ServerMode::Tokio)? else {
        eprintln!("skip wallet-c HTTP test: local TCP/HTTP bind is not permitted");
        return Ok(());
    };
    let server = ctx.start_server()?;

    let mpk_binary = read(&mpk_path).expect("read wallet-c.mpk");
    ...
    assert_eq!(apps, json!(["wallet-c"]));

    let procedures = ctx.get_json("/mudu/app/list/wallet-c")?;
    let procedure_list = procedures["procedures"].as_array().expect("procedure list");
    assert!(procedure_list.contains(&json!("wallet_c/create_user")));
    assert!(procedure_list.contains(&json!("wallet_c/deposit")));
    assert!(procedure_list.contains(&json!("wallet_c/withdraw")));
    assert!(procedure_list.contains(&json!("wallet_c/transfer_funds")));
    assert!(procedure_list.contains(&json!("wallet_c/balance")));
    assert!(procedure_list.contains(&json!("wallet_c/update_profile")));
    ...
    // An overdrawing withdrawal must raise inside the C guest and surface
    ...
    assert!(ctx.user_exists("wallet-c", 3)?);
    assert_eq!(ctx.user_name("wallet-c", 3)?, "Carol");
    assert_eq!(ctx.wallet_balance("wallet-c", 1)?, 9750);
    assert_eq!(ctx.wallet_balance("wallet-c", 2)?, 10000);
    ...
    // DataTypeParamRecord into the positional record envelope, the C guest
    ...
    let profile = ctx.profile_row("wallet-c", 1)?;
    assert_eq!(
        profile,
        Some((
    ...
    assert_eq!(ctx.profile_tags("wallet-c", 1)?, vec!["c".to_string()]);
...
impl TestContext {
    fn new(server_mode: ServerMode) -> RS<Option<Self>> {
        ...
        let base_dir = temp_dir("mududb-testing-wallet-c");
    ...
    fn invoke(&self, proc_name: &str, params: Value) -> RS<Value> {
        self.post_json(
            &format!("/mudu/app/invoke/wallet-c/wallet_c/{proc_name}"),
            params,
        )
    }
