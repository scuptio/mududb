...
fn repo_root() -> PathBuf {
    // Walk up to the repository root (the `.project.home` marker) so the
    // shared target directory is found regardless of the crate's depth in
    // the crates/<group>/<crate> layout.
    let mut dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    loop {
        if dir.join(".project.home").exists() {
            return dir;
        }
        if !dir.pop() {
            panic!("no ancestor of CARGO_MANIFEST_DIR contains a .project.home marker");
        }
    }
}
...
fn run_tpcc_new_order_lockfree_stock(server_mode: ServerMode) -> RS<()> {
    ...
    // Mirrors the bench harness: the server's mpk_path starts EMPTY and the
    // package is installed only after the server is up (like the bench's HTTP
    // install). Startup auto-load from a pre-populated mpk_path used to run
    // package initdb against the not-yet-listening server and fail; that
    // path is now fixed (initdb is deferred and drained once the kernel is
    // RPC-ready) and is covered by testing/tests/test_startup_mpk_autoload.rs.
    // This test intentionally keeps exercising the install-after-start flow.
