...
impl RuntimeSimple {
    ...
    pub async fn initialize(&mut self) -> RS<()> {
        ...
        load_package_files(&self.package_path, async |path| {
            self.init_mpk(path, self.rt_opt.defer_initdb).await?;
            Ok(())
        })
        .await?;
        Ok(())
    ...
    async fn init_mpk<P: AsRef<Path>>(&self, path: P, defer_initdb: bool) -> RS<String> {
    ...
    async fn install_pkg<P: AsRef<Path>>(&self, path: P) -> RS<()> {
        // The install path runs while the server is already accepting
        // connections, so initdb executes immediately (no deferral).
        let mpk_name = self.init_mpk(path.as_ref().to_path_buf(), false).await?;
    ...
    /// Applies pending initdb DDL on every loaded app, sequentially.
    ///
    /// A failure on one app does not stop the remaining apps from draining;
    /// the first error is returned after all apps have been attempted.
    pub async fn drain_initdb(&self) -> RS<()> {
        let mut apps = Vec::new();
        let _ = self.apps.iter_sync(|_k, v| {
            apps.push(v.clone());
            true
        });
        let mut first_error = None;
        for app in apps {
            if let Err(e) = app.drain_initdb().await {
                tracing::error!(app = %app.name(), "deferred initdb drain failed: {}", e);
