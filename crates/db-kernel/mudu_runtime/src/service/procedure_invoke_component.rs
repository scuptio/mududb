...
impl ProcedureInvokeComponent {
    pub fn call(
        ...
        if instance.requires_async() {
            // Components importing the async host API get async-required
            // stores: instantiate and call through the async entry points on
            // a dedicated runtime (the historical behavior for this path).
            let thread = mudu_sys::task::sync::spawn_thread(move || {
                let runtime =
                    mudu_sys::task::async_::build_current_thread_runtime().map_err(|e| {
        ...
        // Sync-world components run the whole invocation on a dedicated
        // thread with no async runtime: they may issue WASI host calls whose
        // wasmtime-wasi sync wrappers panic when a Tokio runtime drives the
        // current thread ("Cannot start a runtime from within a runtime";
        // observed with componentize-dotnet guests, whose runtime startup
        // performs WASI calls). See `ProcedureInstancePool::lease_sync`.
        let thread = mudu_sys::task::sync::spawn_thread(move || {
            let mut leased = instance.lease_sync(&name)?;
            leased.set_worker_local(worker_local);
            leased.invoke_sync(param)
        })
        .map_err(|e| mudu_error!(ErrorCode::Thread, "spawn invoke thread error", e))?;
...
mod tests {
    ...
    /// (`crates/sdk/example/py-spike`) exporting `mp2-hello` must instantiate
    /// under the host runtime, and its `mududb:api/system` open/close import
    /// roundtrip must reach the host handler (which answers with a
    /// worker-local-not-configured error frame in this worker-less harness —
    /// itself proof of a working import call path). Ignored by default:
    /// requires the spike package built via `py-spike/build.sh`.
    #[test]
    #[ignore = "requires the componentize-py spike package (py-spike/build.sh)"]
    fn python_guest_spike_hello_open_close_roundtrip() {
        let package = AppPackage::load(
            PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .parent()
                .expect("mudu_runtime has group parent")
                .join("../sdk/example/py-spike/spike.mpk"),
        )
        .unwrap();
        let modules = build_runtime(false).compile_modules(&package).unwrap();
        let module = modules
            .into_iter()
            .find(|(name, _)| name == "spike")
            .expect("spike module")
            .1;
        let proc = module.procedure("hello").unwrap();
        let param = ProcedureParam::new(0, 0, vec![DataValue::from_i32(7)]);
        let result = ProcedureInvokeComponent::call(
            &proc,
            ComponentTarget::P2,
            ProcOpt::default(),
        ...
        let result = result.expect("python spike call should succeed");
        assert_eq!(result.return_list().len(), 1);
        let text = result.return_list()[0].as_string().expect("string return");
        assert!(text.contains("hello-from-py"), "unexpected text: {}", text);
        assert!(text.contains("params:1"), "unexpected text: {}", text);
        // The worker-less harness cannot serve `open`: the host answers with
        // an error frame, which the guest decodes and reports — proof that
        // the import call path works in both directions. (The full
        // open/close roundtrip is covered end-to-end by py_spike_mpk.)
        assert!(
            text.contains("open_close:failed: worker local interface is not configured"),
            "unexpected text: {}",
            text
        );
    }
