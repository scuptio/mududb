...
mod tests {
    ...
    fn write_standard_package_with_manifest(path: &Path, manifest: &[u8]) {
        let file = SFile::create(path).unwrap();
        let mut zip = zip::ZipWriter::new(file);
        let options = zip::write::SimpleFileOptions::default();

        zip.start_file(file_name::PACKAGE_CFG, options).unwrap();
        zip.write_all(standard_cfg()).unwrap();
        zip.start_file(file_name::PROCEDURE_DESC, options).unwrap();
        zip.write_all(&standard_desc()).unwrap();
        zip.start_file(file_name::DDL_SQL, options).unwrap();
        zip.write_all(b"create table t(id integer);\n").unwrap();
        zip.start_file(file_name::INIT_DB_SQL, options).unwrap();
        zip.write_all(b"").unwrap();
        zip.start_file(file_name::PACKAGE_MANIFEST, options)
            .unwrap();
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn load_package_with_matching_syscall_abi_version() {
        let package_file = package_file("abi_ok");
        write_standard_package_with_manifest(
            &package_file,
            br#"{"format_version":1,"syscall_abi_version":1,"files":["package.cfg.json","package.desc.json","ddl.sql","initdb.sql","package.manifest.json","module.wasm"]}"#,
        );
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn load_package_rejects_mismatched_syscall_abi_version() {
        let package_file = package_file("abi_bad");
        write_standard_package_with_manifest(
            &package_file,
            br#"{"format_version":1,"syscall_abi_version":2,"files":["package.cfg.json","package.desc.json","ddl.sql","initdb.sql","package.manifest.json","module.wasm"]}"#,
        );

        let err = AppPackage::load(&package_file).unwrap_err();
        let message = err.to_string();
        assert!(
            message.contains("unsupported syscall ABI version 2"),
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn load_package_without_syscall_abi_version_defaults_to_current() {
        let package_file = package_file("abi_legacy");
        write_standard_package_with_manifest(
            &package_file,
            br#"{"format_version":1,"files":["package.cfg.json","package.desc.json","ddl.sql","initdb.sql","package.manifest.json","module.wasm"]}"#,
        );
