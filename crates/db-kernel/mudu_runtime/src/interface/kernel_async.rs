...
use super::kernel_sync::{
    context_mudu_conn, delete_internal, fetch_internal, fs_fstat_internal_with_worker_local,
    fs_lseek_internal_with_worker_local, get_context, get_internal, has_task_context, put_internal,
    range_internal, require_current_session, require_worker_local, wire_to_attr_datums,
    wire_to_attrs, wire_to_deltas,
};
...
///
/// Fetch only drains the in-memory context cache — no async I/O — so it
/// shares the synchronous implementation. This mirrors sys_interface, whose
/// async `mudu_fetch_bytes` is identical to the sync one.
pub async fn async_fetch_internal(cursor: Vec<u8>) -> Vec<u8> {
    fetch_internal(&cursor)
}
...
async fn _async_close_internal(close_in: Vec<u8>, worker_local: Option<WorkerLocalRef>) -> RS<()> {
    ...
    // Two id domains as in `_close_internal`: task Context ids close the
    // connection's session; open-minted worker session ids close directly.
...
async fn _async_put_internal(put_in: Vec<u8>, _worker_local: Option<WorkerLocalRef>) -> RS<()> {
    let (session_id, key, value) = decode_put_request(&put_in)?;
    let (_context, conn) = context_mudu_conn(session_id.to_oid())?;
    conn.put_async(key, value).await
}
...
async fn _async_range_internal(
    ...
    let (_context, conn) = context_mudu_conn(session_id.to_oid())?;
    let rows = conn.range_async(&start, &end).await?;
    Ok(rows
        .into_iter()
