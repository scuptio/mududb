...
fn repo_root() -> PathBuf {
    // Walk up to the repository root (the `.project.home` marker) so paths
    // to sibling groups and the shared target directory stay correct
    // regardless of the crate's depth in the crates/<group>/<crate> layout.
...
fn ensure_kv_package_built() -> RS<PathBuf> {
    ...
    // The guest package builds for wasm32-wasip2; sanitizer/instrumentation
    // flags from an outer CI test run (e.g. RUSTFLAGS="-Z sanitizer=address")
    // do not apply to that target and must not leak into the nested build.
