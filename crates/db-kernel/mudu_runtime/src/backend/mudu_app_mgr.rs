...
impl MuduProcInvoker {
    ...
    async fn drain_initdb(&self) -> RS<()> {
        let runtime = self.runtime.read()?.clone();
        runtime.drain_initdb().await
    }

    async fn reload(&self) -> RS<()> {
        // The server is already accepting connections when a reload runs (it
        // is driven by the management API), so initdb executes immediately.
        let runtime = create_runtime_from_cfg(&self.cfg, self.async_runtime.clone(), false).await?;
...
impl AsyncFuncInvoker for MuduProcInvoker {
    async fn invoke(
        ...
        let invoke_result = async {
            ...
            let result = if self.enable_async && app.cfg().use_async {
                app.invoke_async(task_id, &mod_name, &proc_name, param, Some(worker_local))
                    .await?
            } else {
                // Sync-world apps (`use_async: false`) take the dedicated
                // sync invoke path even when the server runs in async mode:
                // their guests' host calls must not execute on a Tokio worker
                // thread (see `ProcedureInvokeComponent::call`).
...
impl MuduAppMgr {
    ...
    /// Drains deferred package initdb across every live invoker, one invoker
    /// at a time.
    ///
    /// Startup creates one invoker (with its own runtime and app instances)
    /// per worker while the server's TCP listener is not bound yet, so each
    /// runtime can hold the same app with pending initdb. Draining strictly
    /// sequentially keeps the DDL itself race-free: the first drain applies it
    /// and writes the app's lock sentinel, and later drains re-check the
    /// sentinel in `initdb` and no-op. A failing invoker does not stop the
    /// remaining invokers; the first error is returned at the end.
    pub async fn drain_initdb(&self) -> RS<()> {
        let mut first_error = None;
        for invoker in self.live_invokers()? {
            if let Err(e) = invoker.drain_initdb().await
                && first_error.is_none()
            {
                first_error = Some(e);
            }
...
async fn build_owned_proc_invoker(
    ...
    // Startup auto-load in kernel server modes must not run package initdb
    // against the server's own TCP listener before it is bound; the backend
    // drains the deferred initdb once the kernel reports RPC-ready. Legacy
    // mode connects in-process (LibSQL file I/O) and keeps immediate initdb.
    let defer_initdb = matches!(cfg.server_mode, ServerMode::IOUring | ServerMode::Tokio);
    let runtime = create_runtime_from_cfg(cfg, async_runtime.clone(), defer_initdb).await?;
    let enable_async = cfg.enable_async;
