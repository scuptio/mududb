...
impl DBConnector {
    ...
    pub async fn connect_with_async_runtime(
        ...
        let app_name = opt_app.clone().unwrap_or(String::default());
        ...
        match opt_db_type {
            Some(db_type) => match db_type {
                ...
                DBType::MuduDB => create_mudu_conn(async_runtime, opt_app).await,
            },
...
async fn create_mudu_conn(
    async_runtime: Option<Arc<dyn AsyncIoProvider>>,
    app_name: Option<String>,
) -> RS<DBConn> {
    Ok(DBConn::Async(Arc::new(
        MuduConnAsync::new_with_runtime_and_app(async_runtime, app_name)?,
    )))
}
