...
use crate::{ServeArgs, serve_with_stop, serve_with_stop_and_runner, spawn_signal_listener};
...
fn unique_path(prefix: &str) -> String {
    ...
    mudu_sys::env_var::temp_dir()
        .join(format!("{}_{}.cfg", prefix, uuid))
        .to_string_lossy()
        .into_owned()
}

#[test]
fn serve_with_stop_returns_error_when_config_missing() -> Result<(), Box<dyn std::error::Error>> {
    let cfg_path = unique_path("missing_mudud_cfg");
    let args = ServeArgs {
        cfg_path: Some(cfg_path.clone()),
    };
    let (_notifier, stop_waiter) = notify_wait();

    let result = serve_with_stop_and_runner(args, stop_waiter, |_cfg, _stop| Ok(()));
...
fn serve_with_stop_uses_existing_config() -> Result<(), Box<dyn std::error::Error>> {
    let cfg_path = unique_path("existing_mudud_cfg");
    sync_write(
        &cfg_path,
        br#"
...
fn serve_with_stop_propagates_config_decode_error() -> Result<(), Box<dyn std::error::Error>> {
    let cfg_path = unique_path("invalid_mudud_cfg");
