...
`sys_interface_standalone` provides a standalone debug path for the `sys_interface` syscall APIs.
On non-`wasm32` targets, syscall implementations are routed to `mudu_adapter` instead of returning `NotImplemented`.
...
Depend on `sys_interface_standalone` in the crate that uses the syscall APIs.
Use `sync_api` for synchronous native code, or add `async` and use `async_api` for asynchronous native code.

```toml
[dependencies]
sys_interface_standalone = { path = "../sys_interface_standalone" }
```
...
```toml
[dependencies]
sys_interface_standalone = { path = "../sys_interface_standalone", features = ["async"] }
```

App code that goes through the `mududb` facade can instead enable the
`standalone-adapter` feature on `mududb`; the `mududb::sys_interface` alias then
resolves to `sys_interface_standalone` on native targets:

```toml
[dependencies]
mududb = { path = "../mududb", features = ["standalone-adapter"] }
```

After that, native builds can directly call the standalone adapter through:

- `sys_interface_standalone::sync_api::*` for synchronous code
- `sys_interface_standalone::async_api::*` for asynchronous code
...
```bash
cargo check -p sys_interface_standalone
cargo check -p sys_interface_standalone --features async
```
