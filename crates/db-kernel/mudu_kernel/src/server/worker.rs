...
impl WorkerRuntime {
    ...
    /// `EntityNotFound` when the table does not exist. The bare name resolves
    /// against the schema of the requesting application (search path
    /// `[current_schema, mududb]` inside the catalog).
    async fn relation_table_id(&self, app_name: Option<&str>, table: &str) -> RS<OID> {
        let desc = self
            .meta_mgr()
            .get_table_by_name(&current_schema(app_name), table)
            .await?;
    ...
    pub(crate) async fn relation_get_in_session(
        ...
        let table_id = self.relation_table_id(app_name, table).await?;
    ...
    pub(crate) async fn relation_update_in_session(
        ...
        let opt_update = OptUpdate {
            delta_assignments: deltas
                ...
                .collect(),
            // The relation-update payload carries absolute datums and deltas
            // only; it cannot express a NULL assignment.
    ...
    async fn run_sql_query_with_tx(
        ...
        // the planner) — parsing and binding are skipped entirely. The cache
        // key carries the schema because the same SQL text binds different
        // tables under different application schemas.
        if let Some(plan) = self
            .plan_cache
            .get(current_schema, &text, catalog_version)?
        {
        ...
        let template = {
            ...
            Binder::new(self.meta_mgr(), current_schema.to_string())
                .bind_template(&stmt)
                .await?
        };
        let Some(template) = template else {
            ...
            let result = core
                .query(&stmt, param, tx_mgr, self.contract.clone(), current_schema)
                .await;
            trace.watch("sql.stage", if result.is_ok() { "done" } else { "error" });
            return result;
        };
        ...
        let plan = if self.meta_mgr().catalog_version() == catalog_version {
            self.plan_cache
                .insert(current_schema, text, catalog_version, template)?
        } else {
    ...
    async fn run_sql_execute_with_tx(
        ...
        let Some(template) = template else {
            ...
            let result = core
                .execute(&stmt, param, tx_mgr, self.contract.clone(), current_schema)
                .await;
            trace.watch(
                "procedure.worker_sql_execute.stage",
                if result.is_ok() {
    ...
    pub(crate) async fn query(
        ...
        let current_schema = current_schema(app_name);
        let core = self.sql_core(oid)?;
        if oid == 0 {
            let tx_mgr = self.contract.begin_tx().await?;
            let result = self
                .run_sql_query_with_tx(core, &current_schema, sql, param, tx_mgr.clone())
                .await;
            if result.is_ok() {
                self.contract.commit_tx(tx_mgr).await?;
            } else {
        ...
        let result = self
            .run_sql_query_with_tx(core, &current_schema, sql, param, tx_mgr)
            .await;
        if started_tx {
    ...
    pub(crate) async fn execute(
        ...
        if oid == 0 {
            ...
            let result = self
                .run_sql_execute_with_tx(core, &current_schema, sql, param, tx_mgr.clone())
                .await;
            if result.is_ok() {
        ...
        let result = self
            .run_sql_execute_with_tx(core, &current_schema, sql, param, tx_mgr)
            .await;
        if started_tx {
            let tx_manager = self.session_manager.take_session_tx(oid)?;
            if result.is_ok() {
...
mod tests {
    ...
    async fn sql_execute(worker: &WorkerRuntime, session_id: OID, sql: &str) -> RS<u64> {
        worker
            .execute(session_id, None, Box::new(sql.to_string()), Box::new(()))
            .await
    }
    ...
    async fn sql_query_values(
        ...
        let rs = worker
            .query(session_id, None, Box::new(sql.to_string()), Box::new(()))
            .await?;
