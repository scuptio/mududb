...
mod tests {
    ...
    impl MetaMgr for TestMetaMgr {
        ...
        async fn get_table_by_name(&self, _schema: &str, name: &str) -> RS<Option<Arc<TableDesc>>> {
            Ok(self
                .tables
                .lock()
                .unwrap()
    ...
    fn select(table: &str, terms: Vec<&str>) -> StmtType {
        ...
        stmt.set_table_reference(ObjectRef::unqualified(table.to_string()));
        for term in terms {
    ...
    fn describe_command_returns_empty_descriptor() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            let desc = Describer::describe(meta_mgr().as_ref(), &command(), DEFAULT_SCHEMA)
                .await
                .unwrap();
    ...
    fn describe_select_missing_table_returns_entity_not_found_with_table_name() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            let err = Describer::describe(
                meta_mgr().as_ref(),
                &select("missing_table", vec!["id"]),
                DEFAULT_SCHEMA,
            )
            .await
            .unwrap_err();
