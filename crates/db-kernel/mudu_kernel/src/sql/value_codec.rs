...
impl ValueCodec {
    ...
    ///
    /// A NULL parameter reports the Binary type family and has no binary
    /// layout at all; it is rejected with a clean `NotImplemented` error
    /// (the template engine fills slots to non-NULL values by construction)
    /// instead of panicking in `DataType::default_for`.
    pub(crate) fn binary_from_param(datum: &dyn DatumDyn, data_type: &DataType) -> RS<Buf> {
        let family = datum.type_family()?;
        if family == data_type.type_family() {
            return datum.to_binary(data_type).map(|binary| binary.into());
        }
        // Non-scalar families (Binary, also what a null datum reports) have
        // no parameterized default; use the parameterless form.
        let source_type = if family.is_scalar_type() {
            DataType::default_for(family)
        } else {
            DataType::new_no_param(family)
        };
        let value = datum.to_value(&source_type)?;
        if value.is_null() {
            return Err(mudu_error!(
                ER::NotImplemented,
                "NULL parameters are not supported by the template engine yet"
            ));
        }
        let typed = DataTyped::new(source_type.clone(), value);
