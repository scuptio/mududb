...
pub enum SetValueTemplate {
    /// Absolute value. A slot fills to a non-NULL binary; a `Const(None)`
    /// NULL literal fills to [`BoundSetValue::Null`] (the NOT NULL rejection
    /// and the fs-bound sentinel are resolved at template-bind time).
...
impl TemplateDatum {
    ...
    pub(crate) fn fill(&self, slots: &[ParamSlot], params: &dyn SQLParams) -> RS<Option<Buf>> {
        match self {
                ...
                .map(|(attr, set_value)| {
                    let set_value = match set_value {
                        SetValueTemplate::Absolute(datum) => match datum.fill(slots, params)? {
                            Some(binary) => BoundSetValue::Absolute(binary),
                            None => BoundSetValue::Null,
                        },
