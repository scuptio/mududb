...
mod tests {
    ...
    impl MetaMgr for TestMetaMgr {
        ...
        async fn get_table_by_name(&self, schema: &str, name: &str) -> RS<Option<Arc<TableDesc>>> {
            ...
            let exact = tables
                ...
                .cloned();
            // Mirror the catalog search path: exact (schema, name) hit wins,
            // then fall back to the default schema.
            if exact.is_some() || schema == DEFAULT_SCHEMA {
                return Ok(exact);
            }
            Ok(tables
                .values()
                .find(|table| table.schema() == DEFAULT_SCHEMA && table.name() == name)
                .cloned())
        }
    ...
    fn binder_arc(meta: Arc<TestMetaMgr>) -> Binder {
        Binder::new(meta, DEFAULT_SCHEMA.to_string())
    }

    fn binder_arc_with_schema(meta: Arc<TestMetaMgr>, current_schema: &str) -> Binder {
        Binder::new(meta, current_schema.to_string())
    }

    fn binder() -> Binder {
        binder_arc(Arc::new(TestMetaMgr::new(schema())))
    }

    fn composite_binder() -> Binder {
        binder_arc(Arc::new(TestMetaMgr::new(composite_schema())))
    }

    fn numeric_binder() -> Binder {
        binder_arc(Arc::new(TestMetaMgr::new(numeric_schema())))
    }
    ...
    fn not_null_value_binder() -> Binder {
        ...
        binder_arc(Arc::new(TestMetaMgr::new(SchemaTable::new(
            "users".to_string(),
            vec![id, name],
            vec![0],
            vec![1],
    ...
    fn bind_insert_encodes_numeric_literal_into_declared_column_type() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            ...
            assert_eq!(
                ...
                "coffee".to_string().to_binary(&note_type).unwrap().as_ref()
            );
        })
        .unwrap()
    }
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn bind_insert_string_literal_encodes_without_quotes() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            let bound = binder()
                .bind(parse_stmt("insert into users values (1, 'alice');"), &())
                .await
            ...
            let name_type = DataType::default_for(TypeFamily::String);
            assert_eq!(
                insert.rows[0].value[0].1,
                "alice".to_string().to_binary(&name_type).unwrap().as_ref()
            );
        })
        .unwrap()
    }
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn bind_update_allows_null_for_nullable_value_column() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            let bound = binder()
                .bind(
            ...
            assert!(
                matches!(&update.value[0].1, BoundSetValue::Null),
                "expected NULL assignment, got {:?}",
                update.value[0].1
            );
        })
        .unwrap()
    ...
    fn partitioned_binder() -> Binder {
        ...
        binder_arc(Arc::new(TestMetaMgr::with_rule(
            schema,
    ...
    fn bind_update_marks_null_fs_column_assignment_for_rebind() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            ...
            let bound = binder_arc(Arc::new(TestMetaMgr::new(schema)))
                .bind(
    ...
    fn fs_type_binder(fs_type: FsTypeDesc) -> Binder {
        binder_arc(Arc::new(TestMetaMgr::with_fs_type(schema(), fs_type)))
    }
    ...
    fn bind_bench() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async {
            let binder = binder_arc(Arc::new(TestMetaMgr::new(composite_schema())));
            let sqls = [
    ...
    fn bind_update_accepts_string_encoded_numeric_delta_placeholder() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            ...
            let binder = binder_arc(Arc::new(TestMetaMgr::new(schema)));
    ...
    fn users_schema_in(schema_ns: &str) -> SchemaTable {
        let mut schema = schema();
        schema.set_schema(schema_ns.to_string());
        schema
    }

    fn bound_select_table_id(bound: BoundStmt) -> OID {
        let BoundStmt::Query(BoundQuery::Select(select)) = bound else {
            panic!("expected bound select");
        };
        select.table_id
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn bind_select_resolves_unqualified_in_current_schema() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            let default_users = users_schema_in(DEFAULT_SCHEMA);
            let shop_users = users_schema_in("shop");
            let mgr = TestMetaMgr::new(default_users);
            mgr.create_table(&shop_users).await.unwrap();
            let binder = binder_arc_with_schema(Arc::new(mgr), "shop");

            let bound = binder
                .bind(parse_stmt("select id from users where id = 1;"), &())
                .await
                .unwrap();
            assert_eq!(bound_select_table_id(bound), shop_users.id());
        })
        .unwrap()
    }
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn bind_select_qualified_reference_bypasses_current_schema() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            let default_users = users_schema_in(DEFAULT_SCHEMA);
            let default_oid = default_users.id();
            let shop_users = users_schema_in("shop");
            let mgr = TestMetaMgr::new(default_users);
            mgr.create_table(&shop_users).await.unwrap();
            let binder = binder_arc_with_schema(Arc::new(mgr), "shop");
            ...
            let bound = binder
                .bind(parse_stmt("select id from mududb.users where id = 1;"), &())
                .await
                .unwrap();
            assert_eq!(bound_select_table_id(bound), default_oid);
            let bound = binder
                .bind(parse_stmt("select id from shop.users where id = 1;"), &())
                .await
                .unwrap();
            assert_eq!(bound_select_table_id(bound), shop_users.id());
        })
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn bind_select_unqualified_falls_back_to_default_schema() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            let default_users = users_schema_in(DEFAULT_SCHEMA);
            let default_oid = default_users.id();
            let mgr = TestMetaMgr::new(default_users);
            ...
            // `shop` owns no `users`: the search path falls back to the
            // default schema.
            let bound = binder
                .bind(parse_stmt("select id from users where id = 1;"), &())
                .await
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn bind_create_table_lands_in_current_schema() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            let binder = binder_arc_with_schema(Arc::new(TestMetaMgr::new(schema())), "shop");
            let bound = binder
                .bind(
                    parse_stmt("create table orders (id int primary key, v int);"),
                    &(),
                )
                .await
                .unwrap();
            ...
            assert_eq!(create.schema.schema(), "shop");
            assert_eq!(create.schema.table_name(), "orders");
        })
        .unwrap()
    }
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn bind_insert_resolves_qualified_table_exactly() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            let default_users = users_schema_in(DEFAULT_SCHEMA);
            let default_oid = default_users.id();
            let mgr = TestMetaMgr::new(default_users);
            ...
            // Qualified reference hits the default-schema table exactly even
            // though the current schema is `shop`.
            let bound = binder
                .bind(
                    parse_stmt("insert into mududb.users values (1, 'alice');"),
                    &(),
                )
                .await
                .unwrap();
            ...
            assert_eq!(insert.table_id, default_oid);

            // Unqualified resolves through the fallback to the same table.
            let bound = binder
                .bind(parse_stmt("insert into users values (1, 'alice');"), &())
                .await
                .unwrap();
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn bind_drop_table_qualified_miss_does_not_fall_back() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            let default_users = users_schema_in(DEFAULT_SCHEMA);
            let default_oid = default_users.id();
            let mgr = TestMetaMgr::new(default_users);
            ...
            // `shop.users` does not exist; a qualified DROP must not fall
            // back to the default-schema table.
            let err = binder
                .bind(parse_stmt("drop table shop.users;"), &())
                .await
                .unwrap_err();
            assert_eq!(err.ec(), ErrorCode::EntityNotFound);

            let bound = binder
                .bind(parse_stmt("drop table if exists shop.users;"), &())
                .await
                .unwrap();
            let BoundStmt::Command(BoundCommand::DropTable(drop)) = bound else {
                panic!("expected bound drop table");
            ...
            // Unqualified DROP follows the search path and finds the
            ...
            assert_eq!(drop.oid, Some(default_oid));
        })
        .unwrap()
    }
}
