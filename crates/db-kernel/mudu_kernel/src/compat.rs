...
/// Builds a router that knows about every format owned by the kernel, the
/// binding crate or the contract crates, and installs it as the global
...
pub fn install_compatibility_router() {
    ...
    router.register(
        FormatKind::SyscallPayload,
        mudu_binding::codec::syscall_payload::migrate::identity(),
    );
