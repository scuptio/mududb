...
fn make_param(key: VecDatum, value: VecDatum) -> PUpdateKeyValue {
    PUpdateKeyValue {
...
struct MockXContract {
    update_return: AtomicU64,
}

impl MockXContract {
    fn new(update_return: u64) -> Self {
        Self {
            update_return: AtomicU64::new(update_return),
        }
    }
}
...
impl XContract for MockXContract {
    ...
    async fn update(
        ...
        Ok(self.update_return.load(Ordering::Relaxed) as usize)
    }
    async fn read_key(
        &self,
        _tx_mgr: Arc<dyn TxMgr>,
...
impl MetaMgr for MockMetaMgr {
    ...
    async fn get_table_by_name(&self, _schema: &str, _name: &str) -> RS<Option<Arc<TableDesc>>> {
        Ok(None)
    }
...
#[test]
fn prepare_fails_for_empty_value() {
    let cmd = UpdateKeyValue::new(
        make_param(
            VecDatum::new(vec![(0, datum(1))]),
            VecDatum::new(Vec::new()),
...
#[test]
fn prepare_succeeds_for_non_empty_key_and_value() {
    let cmd = UpdateKeyValue::new(
        make_param(
            VecDatum::new(vec![(0, datum(1))]),
...
#[test]
fn affected_rows_reflects_update_return_value() {
    let cmd = UpdateKeyValue::new(
        make_param(
            VecDatum::new(vec![(0, datum(1))]),
            VecDatum::new(vec![(0, datum(2))]),
        ),
        Arc::new(MockXContract::new(5)),
        Arc::new(MockMetaMgr),
    );
    block_on(async {
        cmd.prepare().await.unwrap();
        cmd.run().await.unwrap();
        assert_eq!(cmd.affected_rows().await.unwrap(), 5);
    });
}
