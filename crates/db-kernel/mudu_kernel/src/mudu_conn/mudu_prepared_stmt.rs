...
pub struct MuduPreparedStmt {
    ...
    /// Application whose schema the statement resolves against (see
...
impl PreparedStmt for MuduPreparedStmt {
    async fn query(&self, params: Box<dyn SQLParams>) -> RS<Arc<dyn ResultSetAsync>> {
        self.worker_local
            .query(
                self.current_oid().await?,
                self.app_name.as_deref(),
                self.sql.clone_boxed(),
                params,
            )
            .await
    }
...
mod tests {
    ...
    fn stmt(
        ...
        MuduPreparedStmt::new(worker, session, sql, empty_desc(), None)
    }
    ...
    fn desc_returns_cached_descriptor() {
        ...
        let sql: Box<dyn SQLStmt> = Box::new("SELECT 1".to_string());
        let desc = empty_desc();
        let stmt = MuduPreparedStmt::new(worker, session, sql, desc.clone(), None);
