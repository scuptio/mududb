...
//! depends on the SQL text, the schema the statement resolves against, and
//! the catalog, so this cache stores [`BoundTemplate`]s (placeholders kept
//! as ordered slots, see `sql::bound_template`) keyed by `(schema, SQL
//! text)`, tagged with the catalog version they were bound against. A lookup
//! carries the current `MetaMgr::catalog_version()`; a version mismatch is a
//! miss and the entry is overwritten after rebinding, which is how DDL
//! invalidates cached templates. The schema sits in the key because the same
//! SQL text binds different tables under different application schemas.
//!
...
//! serialize on one lock. Entries are keyed by `(schema, SQL text)` (the
//! catalog version lives inside the entry) so lookups borrow both without
//! allocating a key.
...
/// FNV-1a over the schema and SQL text, used only for shard selection (same
/// reasoning as the parse cache).
fn shard_index(schema: &str, sql: &str) -> usize {
    let mut hash: u64 = 0xcbf2_9ce4_8422_2325;
    for byte in schema.as_bytes().iter().chain(sql.as_bytes().iter()) {
        hash ^= u64::from(*byte);
...
impl CachedPlan {
    ...
    async fn run_point_update(
        ...
        let mut null_assignments = Vec::new();
        for (attr, set_value) in &template.value {
            match set_value {
                SetValueTemplate::Absolute(datum) => match datum.fill(slots, params)? {
                    Some(binary) => absolute.push((*attr, binary)),
                    None => null_assignments.push(*attr),
                },
        ...
        if absolute.is_empty() && delta_assignments.is_empty() && null_assignments.is_empty() {
            return Err(mudu_error!(
                ErrorCode::EntityNotFound,
...
/// The `(schema, SQL text)` key is nested so lookups borrow both halves
/// without allocating a combined key.
pub(crate) struct PlanCache {
    shards: [SMutex<HashMap<String, HashMap<String, PlanCacheEntry>>>; PLAN_CACHE_SHARDS],
    hits: AtomicU64,
    misses: AtomicU64,
}
...
impl PlanCache {
    ...
    /// Returns the cached plan for `(schema, sql)` when an entry exists and
    /// was bound against `catalog_version`; a version mismatch is a miss (the
    /// stale entry is left in place and overwritten by the next `insert`).
    pub(crate) fn get(
        &self,
        schema: &str,
        sql: &str,
        catalog_version: u64,
    ) -> RS<Option<Arc<CachedPlan>>> {
        let guard = self.shards[shard_index(schema, sql)]
            .lock()
            .map_err(|_| mudu_error!(ErrorCode::Internal, "plan cache lock poisoned"))?;
        match guard.get(schema).and_then(|plans| plans.get(sql)) {
    ...
    /// Caches `template` under `(schema, sql)` and `catalog_version`,
    /// replacing any existing entry (DDL rebinding overwrites the stale
    /// template), and returns the shared plan for immediate execution.
    pub(crate) fn insert(
        &self,
        schema: &str,
        sql: String,
        ...
        let mut guard = self.shards[shard_index(schema, &sql)]
            .lock()
            .map_err(|_| mudu_error!(ErrorCode::Internal, "plan cache lock poisoned"))?;
        let plans = guard.entry(schema.to_string()).or_default();
        if let Some(entry) = plans.get_mut(&sql) {
            entry.catalog_version = catalog_version;
            entry.plan = plan.clone();
        } else if plans.len() < PLAN_CACHE_SHARD_CAPACITY {
            plans.insert(
                sql,
                PlanCacheEntry {
                    catalog_version,
...
mod tests {
    ...
    fn empty_cache_misses() {
        ...
        assert!(cache.get("mududb", "select 1", 0).unwrap().is_none());
    ...
    fn same_version_hits_and_shares_one_plan() {
        ...
        let inserted = cache
            ...
            .unwrap();
        let hit = cache
            .get("mududb", "delete from t where id = ?", 7)
            .unwrap()
            .unwrap();
    ...
    fn version_mismatch_is_miss_and_rebind_overwrites() {
        ...
        cache
            .insert("mududb", sql.clone(), 1, dummy_template(1))
            .unwrap();
        // A DDL bumped the catalog version: the stale entry misses.
        assert!(cache.get("mududb", &sql, 2).unwrap().is_none());
        // Rebinding overwrites the entry under the new version.
        let rebound = cache
            .insert("mududb", sql.clone(), 2, dummy_template(2))
            .unwrap();
        let hit = cache.get("mududb", &sql, 2).unwrap().unwrap();
        assert!(Arc::ptr_eq(&rebound, &hit));
        // The old version still misses.
        assert!(cache.get("mududb", &sql, 1).unwrap().is_none());
    }
    ...
    fn distinct_sql_texts_do_not_collide() {
        let cache = PlanCache::new();
        cache
            .insert("mududb", "a".to_string(), 0, dummy_template(1))
            .unwrap();
        assert!(cache.get("mududb", "b", 0).unwrap().is_none());
        assert!(cache.get("mududb", "a", 0).unwrap().is_some());
    }

    #[test]
    fn same_sql_text_under_different_schemas_does_not_collide() {
        let cache = PlanCache::new();
        let inserted_a = cache
            .insert("app_a", "select 1".to_string(), 0, dummy_template(1))
            .unwrap();
        let inserted_b = cache
            .insert("app_b", "select 1".to_string(), 0, dummy_template(2))
            .unwrap();
        let hit_a = cache.get("app_a", "select 1", 0).unwrap().unwrap();
        let hit_b = cache.get("app_b", "select 1", 0).unwrap().unwrap();
        assert!(Arc::ptr_eq(&inserted_a, &hit_a));
        assert!(Arc::ptr_eq(&inserted_b, &hit_b));
