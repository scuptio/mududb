...
use mysql::{Opts, OptsBuilder, Params, Pool, PoolConstraints, PoolOpts, Row, Value};
use mysql_async::consts::ColumnType as AsyncColumnType;
use mysql_async::prelude::Queryable as AsyncQueryable;
use mysql_async::{
    Conn as AsyncConn, Opts as AsyncOpts, OptsBuilder as AsyncOptsBuilder, Params as AsyncParams,
    Pool as AsyncPool, Row as AsyncRow, Value as AsyncValue,
};
...
pub fn mudu_query<R: Entity>(
    ...
    with_session(oid, |conn| {
        let rows: Vec<Row> = conn
            .exec(sql_text, Params::Positional(values))
...
pub async fn mudu_query_async<R: Entity>(
    ...
    let rows: Vec<AsyncRow> = session
        .conn
        .exec(sql_text, AsyncParams::Positional(values))
        .await
        .map_err(|e| mudu_error!(ErrorCode::Database, "mysql query error", e))?;
...
pub fn mudu_command(oid: OID, sql_stmt: &dyn SQLStmt, params: &dyn SQLParams) -> RS<u64> {
    ...
    let values = sql_params_to_mysql(params)?;
    with_session(oid, |conn| {
        conn.exec_drop(sql_text, Params::Positional(values))
...
pub async fn mudu_command_async(
    ...
    session
        .conn
        .exec_drop(sql_text, AsyncParams::Positional(values))
        .await
        .map_err(|e| mudu_error!(ErrorCode::Database, "mysql command error", e))?;
    ...
    let fields = row
        ...
        .map(|(idx, column)| {
            let ty = match column.column_type() {
                ...
                | ColumnType::MYSQL_TYPE_YEAR => TypeFamily::String,
                // A declared type we do not map: infer from the row value.
                // A NULL value carries no type evidence, so default to
                // String instead of letting one NULL cell corrupt the
                // whole column's inferred type.
                _ => match row.as_ref(idx) {
                    Some(value) if !matches!(value, Value::NULL) => {
                        infer_type_from_mysql_value(value)
                    }
                    _ => TypeFamily::String,
                },
            };
        ...
        row.columns_ref()
            ...
            .map(|(idx, column)| {
                let ty = match column.column_type() {
                    ...
                    AsyncColumnType::MYSQL_TYPE_VARCHAR
                    | AsyncColumnType::MYSQL_TYPE_VAR_STRING
                    | AsyncColumnType::MYSQL_TYPE_STRING
                    | AsyncColumnType::MYSQL_TYPE_JSON
                    | AsyncColumnType::MYSQL_TYPE_ENUM
                    | AsyncColumnType::MYSQL_TYPE_SET
                    | AsyncColumnType::MYSQL_TYPE_DATE
                    | AsyncColumnType::MYSQL_TYPE_NEWDATE
                    | AsyncColumnType::MYSQL_TYPE_DATETIME
                    | AsyncColumnType::MYSQL_TYPE_TIMESTAMP
                    | AsyncColumnType::MYSQL_TYPE_TIME
                    | AsyncColumnType::MYSQL_TYPE_YEAR => TypeFamily::String,
                    // A declared type we do not map: infer from the row
                    // value. A NULL value carries no type evidence, so
                    // default to String instead of letting one NULL cell
                    // corrupt the whole column's inferred type.
                    _ => match row.as_ref(idx) {
                        Some(value) if !matches!(value, AsyncValue::NULL) => {
                            infer_type_from_mysql_async_value(value)
                        }
                        _ => TypeFamily::String,
                    },
                };
...
/// Convert SQL parameters into `mysql_common` bind values (shared by the
/// sync `mysql` and the `mysql_async` paths).
pub(crate) fn sql_params_to_mysql(params: &dyn SQLParams) -> RS<Vec<Value>> {
    ...
    for idx in 0..params.size() {
        ...
        values.push(datum_to_mysql_value(datum)?);
    }
    Ok(values)
}

/// Convert one datum into a `mysql_common` bind value: integers map to
/// `Int`, floats to `Float`/`Double`, strings and blobs to `Bytes`, numeric
/// and date/time values to their plain-text `Bytes` form (MySQL coerces
/// per statement context), and NULL to `Value::NULL`.
pub(crate) fn datum_to_mysql_value(datum: &dyn DatumDyn) -> RS<Value> {
    ...
    if let Some(v) = value.as_i32() {
        ...
        Err(mudu_error!(
            ErrorCode::NotImplemented,
            format!("unsupported mysql parameter type: {family:?}")
        ))
    }
}
...
fn mysql_async_value_to_data_value(
    ...
    match value {
        AsyncValue::NULL => Ok(DataValue::null()),
