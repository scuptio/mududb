...
fn parser_reports_error_for_invalid_token() {
    ...
    "#;
    let parser = WitParser::new();
    let err = parser.parse_text(source).unwrap_err();
    // Parse errors must propagate with a useful location, not yield a partial
    // result with a success status.
    let message = format!("{}", err);
    assert!(message.contains("row"), "unexpected message: {}", message);
    assert!(
...
#[test]
fn parser_maps_s8_to_i8_scalar() {
    let source = r#"
        package test:api;
        interface binding {
            record packet { tag: s8 }
        }
    "#;
    let parser = WitParser::new();
    let wit = parser.parse_text(source).unwrap();
...
#[test]
fn parser_assigns_one_based_field_numbers_in_declaration_order() {
    let source = r#"
        package test:api;
        interface binding {
            record point { x: s32, y: s32, z: s32 }
            table my-table {
                key: { id: s64 },
                value: { name: string, age: u32 }
            }
            add: func(a: s32, b: s32) -> s32;
        }
    "#;
    ...
    let record_numbers: Vec<u32> = wit.records[0]
        .record_fields
        .iter()
        .map(|f| f.rf_number)
        .collect();
    ...
    let key_numbers: Vec<u32> = wit.tables[0]
        .table_key
        .iter()
        .map(|f| f.rf_number)
        .collect();
    let value_numbers: Vec<u32> = wit.tables[0]
        .table_value
        .iter()
        .map(|f| f.rf_number)
        .collect();
    assert_eq!(key_numbers, vec![1]);
    assert_eq!(value_numbers, vec![1, 2]);

    let param_numbers: Vec<u32> = wit.functions[0]
        .params
        .iter()
        .map(|f| f.rf_number)
        .collect();
    let return_numbers: Vec<u32> = wit.functions[0]
        .returns
        .iter()
        .map(|f| f.rf_number)
        .collect();
    assert_eq!(param_numbers, vec![1, 2]);
    assert_eq!(return_numbers, vec![1]);
}
