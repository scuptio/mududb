...
impl TableInfo {
    ...
    pub fn from(
        table_def: UniTableDef,
        lang: LangKind,
        type_kinds: &HashMap<String, String>,
    ) -> RS<Self> {
        let table_name = to_pascal_case(&table_def.table_name);
        let key_fields = to_field_info(&table_def.table_key, &lang, type_kinds)?;
        let value_fields = to_field_info(&table_def.table_value, &lang, type_kinds)?;
...
mod tests {
    ...
    fn from_normalizes_table_metadata() -> RS<()> {
        let table_def = UniTableDef {
            ...
            table_key: vec![RecordField::new(
                "id".to_string(),
                "id".to_string(),
                UniDataType::Scalar(UniScalar::I32),
            )],
            table_value: vec![RecordField::new(
                "name".to_string(),
                "name".to_string(),
                UniDataType::Scalar(UniScalar::String),
            )],
        };
        let info = TableInfo::from(table_def, LangKind::CSharp, &Default::default())?;
