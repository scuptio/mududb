...
#[test]
fn args_parses_python_alias() -> Result<(), Box<dyn Error>> {
    let args = Args::try_parse_from(["mtp", "-i", "in.py", "-o", "out_gen.py", "py"])?;
    assert!(matches!(args.command, CommandType::Python));
    assert_eq!(args.input, "in.py");
    assert_eq!(args.output, "out_gen.py");
    Ok(())
}
...
#[test]
fn args_parses_repeatable_type_wit() -> Result<(), Box<dyn Error>> {
    let args = Args::try_parse_from([
        "mtp",
        "-i",
        "in.go",
    ...
    assert_eq!(
        args.type_wit,
        vec!["types.wit".to_string(), "more_types/".to_string()]
    );
    assert!(matches!(args.command, CommandType::Go));

    let args = Args::try_parse_from(["mtp", "-i", "in.go", "-o", "out.go", "go"])?;
    assert!(args.type_wit.is_empty());
    Ok(())
}
