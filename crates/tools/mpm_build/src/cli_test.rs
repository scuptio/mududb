//! Integration tests for the `mpm-build` command-line parser and subcommands.
#![allow(missing_docs)]
...
fn parse_create_success() -> Result<()> {
    ...
    let MpmBuildCommand::Package(pkg) = cmd else {
...
fn parse_create_rejects_missing_required_argument() {
    let result = parse_arguments_from(["mpm-build", "create", "--package-cfg", "/tmp/cfg.json"]);
...
fn parse_merge_desc_success() -> Result<()> {
    ...
    let MpmBuildCommand::MergeDesc(merge) = cmd else {
...
fn parse_merge_desc_rejects_missing_argument() {
    let result = parse_arguments_from(["mpm-build", "merge-desc", "--input-folder", "/tmp/input"]);
    assert!(result.is_err());
}
...
fn parse_rejects_unknown_subcommand() {
    let result = parse_arguments_from(["mpm-build", "unknown", "--foo", "bar"]);
    assert!(result.is_err());
}

#[test]
fn parse_rejects_unknown_flag() {
    let result = parse_arguments_from(["mpm-build", "merge-desc", "--unknown"]);
    assert!(result.is_err());
}
