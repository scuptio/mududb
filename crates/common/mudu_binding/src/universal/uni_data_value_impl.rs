...
impl UniDataValue {
    pub fn uni_to(self) -> RS<DataValue> {
        let value = match self {
            UniDataValue::Scalar(value) => match value {
                ...
                UniScalarValue::Null => DataValue::null(),
            },
    ...
    pub fn uni_from(data_value: DataValue) -> RS<UniDataValue> {
        // `DataValue::null()` reports `TypeFamily::Binary` (a pinned quirk of
        // the value semantics), so the null check must come before the
        // family dispatch, or it would hit the binary conversion and panic.
        if data_value.is_null() {
            return Ok(UniDataValue::from_scalar(UniScalarValue::Null));
        }
...
mod tests {
    ...
    fn supported_scalar_uni_to_from_roundtrip() {
        ...
        assert_scalar_uni_to_from_roundtrip(UniDataValue::Scalar(UniScalarValue::from_i128(
            (-128i128).to_be_bytes().to_vec(),
        )));
        assert_scalar_uni_to_from_roundtrip(UniDataValue::Scalar(UniScalarValue::from_u128(
            128u128.to_be_bytes().to_vec(),
        )));
    ...
    #[test]
    fn null_scalar_uni_to_produces_data_value_null() {
        let value = UniDataValue::Scalar(UniScalarValue::Null);
        let dat = value.uni_to().unwrap();
        assert!(dat.is_null());
    }

    #[test]
    fn null_scalar_uni_to_from_roundtrip() {
        let value = UniDataValue::Scalar(UniScalarValue::Null);
        let dat = value.clone().uni_to().unwrap();
        assert!(dat.is_null());
        let back = UniDataValue::uni_from(dat).unwrap();
        assert!(
            matches!(back.as_scalar().unwrap(), UniScalarValue::Null),
            "expected the Null scalar arm after the round-trip, got {back:?}"
        );
        // Also pins the hand-written serde tag-21 arm used by the JSON paths.
        assert_eq!(serialize_json(&back), serialize_json(&value));
        let restored: UniDataValue =
            mudu::common::serde_utils::deserialize_from_json(&serialize_json(&value)).unwrap();
        assert!(
            matches!(restored.as_scalar().unwrap(), UniScalarValue::Null),
            "expected the Null scalar arm after the serde round-trip"
        );
    }

    #[test]
    fn uni_from_null_data_value_maps_to_null_scalar() {
        // Regression: this used to hit the Binary family dispatch and panic
        // in `into_binary`.
        let uni = UniDataValue::uni_from(DataValue::null()).unwrap();
        assert!(matches!(uni.as_scalar().unwrap(), UniScalarValue::Null));
    }

    #[test]
    fn non_null_scalars_still_roundtrip_after_null_support() {
        // Guard against the null pre-check shadowing real values.
        for value in [
            UniDataValue::Scalar(UniScalarValue::from_i32(0)),
            UniDataValue::Scalar(UniScalarValue::from_string(String::new())),
            UniDataValue::Scalar(UniScalarValue::from_blob(Vec::new())),
        ] {
            let dat = value.clone().uni_to().unwrap();
            assert!(!dat.is_null(), "{value:?} must not decode as null");
            let back = UniDataValue::uni_from(dat).unwrap();
            assert_eq!(serialize_json(&back), serialize_json(&value));
        }
    }
