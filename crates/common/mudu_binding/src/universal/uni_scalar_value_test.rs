...
expect_ok_test!(expect_i8_ok, expect_i8, from_i8, 42i8, i8);
...
#[test]
fn expect_u128_ok() {
    // The generated type stores the 128-bit payload as big-endian bytes.
    let bytes = 42u128.to_be_bytes().to_vec();
    let value = UniScalarValue::from_u128(bytes.clone());
    assert_eq!(value.expect_u128(), &bytes);
}

#[test]
fn expect_i128_ok() {
    let bytes = (-42i128).to_be_bytes().to_vec();
    let value = UniScalarValue::from_i128(bytes.clone());
    assert_eq!(value.expect_i128(), &bytes);
}

#[test]
fn expect_blob_ok() {
    let value = UniScalarValue::from_blob(vec![1, 2, 3]);
    assert_eq!(value.expect_blob(), &vec![1, 2, 3]);
}
...
fn expect_timestamptz_ok() {
    let value = UniScalarValue::from_timestamp_tz("2026-05-20T14:30:00+08:00".to_string());
    assert_eq!(value.expect_timestamp_tz(), "2026-05-20T14:30:00+08:00");
}
