...
use mudu_type::data_type_param_string::DataTypeParamString;
use mudu_type::data_type_param_time::DataTypeParamTime;
use mudu_type::data_type_param_timestamp::DataTypeParamTimestamp;
use mudu_type::data_type_param_timestamptz::DataTypeParamTimestampTz;
...
impl UniDataType {
    ...
    pub fn uni_to_with_params(self, params: Option<Vec<UniDataValue>>) -> RS<DataType> {
        let ty = match self {
            UniDataType::Scalar(scalar) => scalar_with_params_to(scalar, params)?,
            UniDataType::Array(inner) => {
                let ty = *inner;
                let inner_ty = ty.uni_to()?;
                let array_kind =
                    DataTypeParamKind::Array(Box::new(DataTypeParamArray::new(inner_ty)));
                DataType::from_id_param(TypeFamily::Array, Some(array_kind))
            }
            UniDataType::Record(inner) => {
                ...
                let object_kind = DataTypeParamKind::Record(Box::new(DataTypeParamRecord::new(
                    inner.record_name.clone(),
                    fields,
                )));
                DataType::from_id_param(TypeFamily::Record, Some(object_kind))
            }
            _ => {
    ...
    pub fn uni_from(ty: DataType) -> RS<UniDataType> {
        // The Binary family (which is also what a null datum reports)
        // converts through its scalar Blob form.
        let uni_ty = if ty.type_family().is_scalar_type() || ty.type_family() == TypeFamily::Binary
        {
            let scalar = UniScalar::uni_from(ty)?;
            UniDataType::Scalar(scalar)
        } else {
            match ty.type_family() {
                TypeFamily::Array => {
                    let param = ty.into_array_param();
                    let inner_ty = param.into_data_type();
                    let inner_uni_ty = Self::uni_from(inner_ty)?;
                    UniDataType::Array(Box::new(inner_uni_ty))
                }
                TypeFamily::Record => {
...
fn scalar_with_params_to(scalar: UniScalar, params: Option<Vec<UniDataValue>>) -> RS<DataType> {
    match (scalar, params) {
        (UniScalar::String, Some(params)) if !params.is_empty() => {
            let Some(UniDataValue::Scalar(UniScalarValue::I64(length))) = params.first().cloned()
            else {
            ...
            Ok(DataType::from_string(DataTypeParamString::new(
                length as u32,
            )))
        }
        (UniScalar::Numeric, Some(params)) if !params.is_empty() => {
            ...
            let param = DataTypeParamNumeric::new(precision as u8, scale as u8);
            param
                .validate()
        ...
        (UniScalar::Time, Some(params)) if !params.is_empty() => {
            ...
            let param = DataTypeParamTime::new(precision as u8);
            param
                .validate()
                .map_err(|message| mudu_error!(ErrorCode::TypeConversionFailed, message))?;
        ...
        (UniScalar::Timestamp, Some(params)) if !params.is_empty() => {
            ...
            let param = DataTypeParamTimestamp::new(precision as u8);
            param
                .validate()
                .map_err(|message| mudu_error!(ErrorCode::TypeConversionFailed, message))?;
        ...
        (UniScalar::TimestampTz, Some(params)) if !params.is_empty() => {
            ...
            let param = DataTypeParamTimestampTz::new(precision as u8);
            param
                .validate()
                .map_err(|message| mudu_error!(ErrorCode::TypeConversionFailed, message))?;
...
fn extract_param_i64(params: &[UniDataValue], index: usize, name: &str) -> RS<i64> {
    let value = params
        .get(index)
        .ok_or_else(|| mudu_error!(ErrorCode::TypeConversionFailed, format!("missing {}", name)))?;
    let UniDataValue::Scalar(UniScalarValue::I64(value)) = value else {
        return Err(mudu_error!(
            ErrorCode::TypeConversionFailed,
...
fn _rewrite_inline(vec_ty: Vec<UniDataType>) -> RS<Vec<UniDataType>> {
    ...
    let mut records = HashMap::<String, UniDataType>::new();
...
fn build_inline(ty: &mut UniDataType, records: &mut HashMap<String, UniDataType>) -> RS<()> {
    match ty {
        UniDataType::Identifier(record_name) => {
...
fn visit_ty(
    ...
    match ty {
        UniDataType::Identifier(ident_name) => {
        ...
        UniDataType::Box(boxed_type) => {
            visit_ty(index, boxed_type, record_ty, dependency);
        }
        UniDataType::Scalar(_) => {}
        UniDataType::Binary => {}
    }
}
...
mod tests {
    use super::*;
    use crate::universal::uni_data_type_impl::topological_sort;
    ...
    fn test_temporal_scalar_params_parse_and_validate() {
        let cases = vec![
            (UniScalar::Time, 3i64, TypeFamily::Time),
            (UniScalar::Timestamp, 4i64, TypeFamily::Timestamp),
            (UniScalar::TimestampTz, 2i64, TypeFamily::TimestampTz),
        ];

        for (scalar, precision, expected_id) in cases {
            let ty = scalar_with_params_to(
                scalar,
                Some(vec![UniDataValue::Scalar(UniScalarValue::I64(precision))]),
            )
            .unwrap();
            assert_eq!(ty.type_family(), expected_id);
        }
    }
    ...
    fn test_temporal_scalar_params_reject_invalid_values() {
        let err = scalar_with_params_to(
            UniScalar::Time,
            Some(vec![UniDataValue::Scalar(UniScalarValue::I64(-1))]),
        )
        .unwrap_err();
        ...
        let err = scalar_with_params_to(
            UniScalar::Timestamp,
            Some(vec![UniDataValue::Scalar(UniScalarValue::I64(7))]),
        )
        .unwrap_err();
        ...
        let err = scalar_with_params_to(
            ...
            Some(vec![UniDataValue::Scalar(UniScalarValue::String(
                "x".to_string(),
            ))]),
        )
        .unwrap_err();
