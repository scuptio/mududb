...
use crate::universal::mp_wire::{FromValue, ToValue, WireError, decode_value, encode_value};
...
fn wire_err(e: WireError) -> MuduError {
...
fn result_set_to_mu(rs: ResultBatch) -> RS<UniResultSet> {
    ...
    let cursor = encode_value(&mu_oid.to_value());
...
fn result_set_from_mu(rs: UniResultSet) -> RS<ResultBatch> {
    ...
    let mu_oid =
        UniOid::from_value(&decode_value(&rs.cursor).map_err(wire_err)?).map_err(wire_err)?;
...
pub(crate) fn tuple_desc_from_mu(desc: UniRecordType) -> RS<TupleFieldDesc> {
...
pub(crate) fn tuple_desc_to_mu(desc: TupleFieldDesc) -> RS<UniRecordType> {
