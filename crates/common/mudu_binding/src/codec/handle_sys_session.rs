...
pub(crate) fn decode_error_result(input: &[u8]) -> RS<()> {
    ...
    let error = UniError::from_value(
        &decode_value(&input[ERROR_MAGIC.len()..])
            .map_err(|e| mudu::mudu_error!(mudu::error::ErrorCode::Decode, e.message))?,
    )
    .map_err(|e| mudu::mudu_error!(mudu::error::ErrorCode::Decode, e.message))?;
...
pub fn serialize_error_result(error: MuduError) -> Vec<u8> {
    ...
    let encoded_error = encode_value(&adapter::error_to_mu(error).to_value());
...
pub fn deserialize_open_param(input: &[u8]) -> RS<UniSessionOpenArgv> {
    ...
    let argv = UniSessionOpenArgv::from_value(
        &decode_value(input)
            .map_err(|e| mudu::mudu_error!(mudu::error::ErrorCode::Decode, e.message))?,
    )
    .map_err(|e| mudu::mudu_error!(mudu::error::ErrorCode::Decode, e.message))?;
