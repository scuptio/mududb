...
fn timestamptz_type(precision: u8) -> DataType {
    DataType::from_timestamptz(DataTypeParamTimestampTz::new(precision))
}
...
fn timestamptz_len_and_output_len_are_eight_bytes() {
    let ty = timestamptz_type(6);
    let value = DataValue::from_timestamptz(TimestampTzValue::from_epoch_micros_utc(0));
...
fn timestamptz_order_equal_and_hash() {
    let a = DataValue::from_timestamptz(TimestampTzValue::from_epoch_micros_utc(100));
    let b = DataValue::from_timestamptz(TimestampTzValue::from_epoch_micros_utc(200));
    let a2 = DataValue::from_timestamptz(TimestampTzValue::from_epoch_micros_utc(100));
