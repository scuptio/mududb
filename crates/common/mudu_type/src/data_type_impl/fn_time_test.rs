...
fn time_type(precision: u8) -> DataType {
    DataType::from_time(DataTypeParamTime::new(precision))
}
...
fn time_len_and_output_len_are_eight_bytes() {
    let ty = time_type(6);
    let value = DataValue::from_time(TimeValue::parse("12:34:56").unwrap());
...
fn time_order_equal_and_hash() {
    let a = DataValue::from_time(TimeValue::parse("10:00:00").unwrap());
    let b = DataValue::from_time(TimeValue::parse("11:00:00").unwrap());
    let a2 = DataValue::from_time(TimeValue::parse("10:00:00").unwrap());
