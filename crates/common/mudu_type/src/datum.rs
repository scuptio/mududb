...
impl<T: Datum> DatumDyn for Option<T> {
    fn type_family(&self) -> RS<TypeFamily> {
        Ok(T::data_type().type_family())
    }

    fn to_binary(&self, data_type: &DataType) -> RS<DataBinary> {
        match self {
            Some(value) => value.to_binary(data_type),
            None => Err(mudu_error!(
                ErrorCode::InvalidType,
                "NULL has no binary encoding; pass NULL through nullable tuple fields"
            )),
        }
    }
    ...
    fn to_textual(&self, data_type: &DataType) -> RS<DataTextual> {
        match self {
            Some(value) => value.to_textual(data_type),
            None => Ok(DataTextual::from("NULL".to_string())),
        }
    }

    fn to_value(&self, data_type: &DataType) -> RS<DataValue> {
        match self {
            Some(value) => value.to_value(data_type),
...
impl<T: Datum> Datum for Option<T> {
    ...
    fn from_textual(textual: &str) -> RS<Self> {
        Ok(Some(T::from_textual(textual)?))
    }
}
