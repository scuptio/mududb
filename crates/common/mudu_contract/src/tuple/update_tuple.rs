...
use crate::tuple::read_datum::{read_binary_data, read_slot};
...
pub fn update_tuple(
    ...
    if field.is_fixed_len() {
        ...
        if value.len() == slot.length() {
            // Same-length splice: the slot (offset, length) is unchanged and
            // trailing slots stay valid, so only the payload is replaced.
            let up_data =
