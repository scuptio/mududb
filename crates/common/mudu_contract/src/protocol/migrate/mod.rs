//! Migration handlers for the wire protocol frame format.
//!
//! The module lives next to the protocol frame encode/decode implementation so
//! that the protocol crate owns its own compatibility story.
...
mod tests {
    ...
    fn identity_handler_clones_input() -> Result<(), MigrateError> {
        ...
        router.set_supported_window(FormatKind::ProtocolFrame, 1, 1);
        router.register(FormatKind::ProtocolFrame, identity());
        let input = vec![0xABu8, 0xCD, 0xEF];
        let output =
            router.migrate(FormatKind::ProtocolFrame, 1, 1, &input, &NoopOptionProvider)?;
