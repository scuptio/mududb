...
#[cfg(all(feature = "component-model", not(feature = "async")))]
/// Execute a query from a serialized byte payload against the host interface.
pub fn mudu_query_bytes(query_in: &[u8]) -> RS<Vec<u8>> {
    Ok(crate::inner_component::inner_query_bytes(query_in))
}

#[cfg(all(feature = "component-model", not(feature = "async")))]
/// Execute a command from a serialized byte payload against the host interface.
pub fn mudu_command_bytes(command_in: &[u8]) -> RS<Vec<u8>> {
    Ok(crate::inner_component::inner_command_bytes(command_in))
}

#[cfg(all(feature = "component-model", not(feature = "async")))]
/// Execute a batch of statements from a serialized byte payload against the
/// host interface.
pub fn mudu_batch_bytes(batch_in: &[u8]) -> RS<Vec<u8>> {
    Ok(crate::inner_component::inner_batch_bytes(batch_in))
}
