...
/// Forward a serialized `query` payload to the component-model host interface
/// and return the host's raw response bytes.
pub fn inner_query_bytes(query_in: &[u8]) -> Vec<u8> {
    system::query(query_in)
}

/// Forward a serialized `command` payload to the component-model host
/// interface and return the host's raw response bytes.
pub fn inner_command_bytes(command_in: &[u8]) -> Vec<u8> {
    system::command(command_in)
}

/// Forward a serialized `batch` payload to the component-model host interface
/// and return the host's raw response bytes.
pub fn inner_batch_bytes(batch_in: &[u8]) -> Vec<u8> {
    system::batch(batch_in)
}
