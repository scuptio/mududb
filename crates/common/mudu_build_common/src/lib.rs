...
/// Returns the repository root directory, located by walking up from
/// `CARGO_MANIFEST_DIR` to the nearest ancestor containing the
/// `.project.home` marker file (the same anchor `mudu_utils::this_file!`
/// uses). This keeps build scripts independent of how deep the crate sits
/// in the `crates/<group>/<crate>` layout.
pub fn repo_root() -> Result<PathBuf> {
    let manifest_dir =
        env_var::var("CARGO_MANIFEST_DIR").ok_or_else(|| error("CARGO_MANIFEST_DIR not set"))?;
    let mut dir = PathBuf::from(manifest_dir);
    loop {
        if sync_path_exists(dir.join(".project.home")) {
            return Ok(dir);
        }
        if !dir.pop() {
            return Err(error(
                "no ancestor of CARGO_MANIFEST_DIR contains a .project.home marker",
            ));
        }
    }
}
