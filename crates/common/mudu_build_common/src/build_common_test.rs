...
mod tests {
    ...
    fn sample_versions() -> BTreeMap<String, String> {
        ...
        versions.insert("rmp".to_string(), "0.8.15".to_string());
    ...
    #[test]
    fn repo_root_finds_project_home_marker() -> crate::Result<()> {
        let manifest_dir = std::env::var("CARGO_MANIFEST_DIR")?;
        let root = repo_root()?;
        assert!(root.join(".project.home").is_file());
        assert!(Path::new(&manifest_dir).starts_with(&root));
        Ok(())
    }
