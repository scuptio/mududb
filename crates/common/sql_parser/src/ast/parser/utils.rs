...
/// Strip the surrounding quotes from a SQL string literal and collapse the
/// SQL escape of the quote character itself (`''` → `'`, `""` → `"`).
/// Text that is not a matched quoted literal is returned unchanged.
pub(crate) fn unquote_sql_literal(raw: &str) -> String {
