...
impl SQLParser {
    ...
    pub(crate) fn visit_object_reference(
        &self,
        context: &ParseContext,
        node: Node,
    ) -> RS<ObjectRef> {
        ...
        let schema = match node.child_by_field_name(ts_field_name::SCHEMA_NAME) {
            Some(n_schema_name) => {
                Some(ts_node_context_string(context.parse_str(), &n_schema_name)?)
            }
            None => None,
        };
        Ok(ObjectRef::new(schema, name))
    }
    ...
    pub(crate) fn visit_invocation(&self, context: &ParseContext, node: Node) -> RS<ExprFunction> {
        ...
        for i in 0..node.child_count() {
            ...
            if child.kind().eq("object_reference") {
                name = Some(self.visit_object_reference(context, child)?.name().clone());
                break;
            }
        }
        let name = match name {
