...
impl SQLParser {
    ...
    pub(crate) fn try_parse_custom_statement(&self, sql: &str) -> RS<Option<StmtList>> {
        ...
        // The custom path handles exactly one statement; a multi-statement
        // script must go through the mixed-script splitter instead, or all
        // statements after the first one would be swallowed.
        if split_top_level_statements(sql).len() != 1 {
            return Ok(None);
        }
...
/// `--` line comments and `/* */` block comments, and respecting
/// single/double-quoted strings. Block comments do not nest (standard SQL);
/// an unterminated block comment swallows the rest of the script.
pub(crate) fn split_top_level_statements(sql: &str) -> Vec<String> {
    ...
    while let Some(ch) = chars.next() {
        ...
        if !in_single_quote && !in_double_quote && ch == '/' && chars.peek() == Some(&'*') {
            // Block comment: skip through the closing `*/`, or to the end of
            // the script when the comment is unterminated.
            chars.next();
            let mut prev = '\0';
            for skipped in chars.by_ref() {
                if prev == '*' && skipped == '/' {
