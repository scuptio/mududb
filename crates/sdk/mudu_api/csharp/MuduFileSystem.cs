...
public static class MuduFileSystem
{
    ...
    /// Canonical cross-language name of <see cref="OpenReadOnly"/> (FS_O_RDONLY=0).
    public const uint FS_O_RDONLY = OpenReadOnly;

    /// Canonical cross-language name of <see cref="OpenWriteOnly"/> (FS_O_WRONLY=1).
    public const uint FS_O_WRONLY = OpenWriteOnly;

    /// Canonical cross-language name of <see cref="OpenReadWrite"/> (FS_O_RDWR=2).
    public const uint FS_O_RDWR = OpenReadWrite;

    /// Canonical cross-language seek whence: from the start of the file (FS_SEEK_SET=0).
    public const uint FS_SEEK_SET = 0;

    /// Canonical cross-language seek whence: from the current position (FS_SEEK_CUR=1).
    public const uint FS_SEEK_CUR = 1;

    /// Canonical cross-language seek whence: from the end of the file (FS_SEEK_END=2).
    public const uint FS_SEEK_END = 2;
    ...
    private static FsResponse<T> FromResult<T>(SyscallResult<T> result)
    {
