...
fn create_update_and_delete_user() {
    ...
    assert_eq!(db.query_wallet(3).unwrap().balance, Some(0));
...
fn transfer_funds_moves_balance_and_writes_transaction() {
    ...
    assert_eq!(db.query_wallet(1).unwrap().balance, Some(9500));
    assert_eq!(db.query_wallet(2).unwrap().balance, Some(10500));
    assert_eq!(
        db.query_count(
...
fn deposit_withdraw_and_purchase_update_balance_and_transactions() {
    ...
    assert_eq!(db.query_wallet(1).unwrap().balance, Some(10100));
    assert_eq!(
        db.query_count(
...
impl TestDb {
    ...
    fn query_wallet(&self, user_id: i32) -> Option<Wallets> {
        let rs: EntitySet<Wallets> = self.query_records(Wallets::SQL_GET_BY_PK, &(user_id,));
    ...
    >(
        ...
        sql: &str,
        params: &P,
    ) -> mududb::contract::database::entity_set::EntitySet<R> {
