...
def find_project_root() -> Path:
    """Find the repository root by looking for the .project.home marker file."""
    path = Path(__file__).resolve().parent
    for parent in [path, *path.parents]:
        if (parent / ".project.home").exists():
            return parent
    raise RuntimeError("Cannot find repository root (no .project.home marker in ancestry)")
...
class PostgresProcedureBackend(PostgresBackend):
    ...
    installs PL/pgSQL procedures (crates/sdk/example/tpcc/sql/procedures_postgres.sql)
...
def newest_source_mtime(project_root: Path) -> float:
    ...
    skip_trees = [
        project_root / "crates" / "sdk" / "example" / "tpcc" / "src" / "rust",
        project_root / "crates" / "sdk" / "example" / "tpcc" / "src" / "bin",
    ]
...
class MududbBackend(BenchmarkBackend):
    ...
    def _build_mpk(self) -> Path:
        tpcc_dir = self.project_root / "crates" / "sdk" / "example" / "tpcc"
...
class SpacetimeDbBackend(BenchmarkBackend):
    ...
    def _stdb_dir(self) -> Path:
        return self.project_root / "crates" / "sdk" / "example" / "tpcc" / "spacetimedb"
        ...
        for d in (module_dir, client_dir):
            if not (d / "Cargo.toml").exists():
                raise RuntimeError(
                    ...
                    "The Rust module/client under crates/sdk/example/tpcc/spacetimedb/ "
                    "is not ready yet."
                )
...
class ParamikoSsh:
    ...
    def put_config(self, content: str) -> str:
        ...
        remote_path = (
            f"{self.remote.server_project_root}/crates/sdk/example/tpcc/bench_remote_agent.yaml"
        )
    ...
    def start_server_run(self, backend: str, cores: int) -> None:
        ...
        command = (
            f"cd {shlex.quote(root)}/crates/sdk/example/tpcc && "
...
# crates/sdk/mudu_api/rust/target) and are gigabytes each.
_SYNC_EXCLUDE_ANY_DEPTH = {"target", "__pycache__"}
# crates/sdk/example/tpcc/spacetimedb must stay synced: in remote mode the server-side
