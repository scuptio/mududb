...
pub async fn rel_get(
    ...
    if RELATION_SUPPORT.load(Ordering::Relaxed) == RELATION_SUPPORT_SQL {
        return rel_get_sql(xid, table, key, select).await;
    }
    match mudu_relation_get(xid, table, key, select).await {
        Err(error) if is_not_implemented(&error) => {
            RELATION_SUPPORT.store(RELATION_SUPPORT_SQL, Ordering::Relaxed);
            rel_get_sql(xid, table, key, select).await
        }
        result => {
...
pub async fn rel_get_one(
    xid: OID,
    table: &str,
    key: &[(u64, Vec<u8>)],
    select: &[u64],
) -> RS<Vec<Option<Vec<u8>>>> {
    require_row(rel_get(xid, table, key, select).await?, table)
}

/// Read-modify-write one relation row by primary key.
pub async fn rel_update(
    xid: OID,
    table: &str,
    key: &[(u64, Vec<u8>)],
    values: &[(u64, Vec<u8>)],
    ...
    if RELATION_SUPPORT.load(Ordering::Relaxed) == RELATION_SUPPORT_SQL {
        return rel_update_sql(xid, table, key, values, deltas).await;
    }
    match mudu_relation_update(xid, table, key, values, deltas).await {
        Err(error) if is_not_implemented(&error) => {
            RELATION_SUPPORT.store(RELATION_SUPPORT_SQL, Ordering::Relaxed);
            rel_update_sql(xid, table, key, values, deltas).await
        }
        result => {
            RELATION_SUPPORT.store(RELATION_SUPPORT_NATIVE, Ordering::Relaxed);
            result
...
pub async fn rel_insert(
    ...
    if RELATION_SUPPORT.load(Ordering::Relaxed) == RELATION_SUPPORT_SQL {
        return rel_insert_sql(xid, table, key, values).await;
    }
    match mudu_relation_insert(xid, table, key, values).await {
        Err(error) if is_not_implemented(&error) => {
            RELATION_SUPPORT.store(RELATION_SUPPORT_SQL, Ordering::Relaxed);
            rel_insert_sql(xid, table, key, values).await
        }
        result => {
            RELATION_SUPPORT.store(RELATION_SUPPORT_NATIVE, Ordering::Relaxed);
            result
    ...
    for delta in deltas {
        ...
        match delta.op {
            ...
            RELATION_DELTA_OP_SUB_WRAP_DEFERRED => {
                ...
                let current_row =
                    require_row(rel_get_sql(xid, table, key, &[delta.attr]).await?, table)?;
