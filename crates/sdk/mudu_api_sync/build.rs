...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Workspace dependency versions are read from the sdk group workspace
    // manifest (the repository root is no longer a cargo workspace).
    let root_manifest_path = repo_root.join("crates").join("sdk").join("Cargo.toml");
    ...
    let universal_src_root = repo_root
        .join("crates")
        .join("common")
        .join("mudu_binding")
        .join("src")
