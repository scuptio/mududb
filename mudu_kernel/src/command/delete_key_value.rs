...
impl DeleteKeyValue {
    pub fn new(
        ...
        Self {
            inner: FMutex::new(_DeleteKeyValue::new(param, x_contract, meta_mgr)),
        }
    }
}
...
impl _DeleteKeyValue {
    ...
    async fn prepare(&self) -> RS<()> {
        ...
        if self.param.key.data().is_empty() {
            return Err(mudu_error!(ER::EntityNotFound, "delete key is empty"));
        }
        Ok(())
    }
