...
use mudu_sys::io::worker_ring::{
    WorkerLocalRing, set_current_worker_ring, unset_current_worker_ring,
};
...
impl WorkerRingLoop {
    ...
    fn start_connection_task(
        ...
        let _ = self.connection_task_fds.lock().unwrap().insert(conn_id, fd);
        task::spawn(
            Some(conn_id),
...
mod tests {
    ...
    async fn test_worker_loop() -> Option<WorkerRingLoop> {
        ...
        .await
        .unwrap();
        let mailbox_fd = mudu_sys::sync::blocking::eventfd().unwrap();
        match WorkerRingLoop::new(WorkerRingLoopArgs {
            ...
            Err(_) => {
                let _ = mudu_sys::sync::blocking::close_fd(mailbox_fd);
                None
            }
        }
    }

    async fn drive_ring_future<T>(loop_state: &mut WorkerRingLoop, handle: &JoinHandle<T>) -> RS<()>
    where
        T: Send + 'static,
    {
    ...
    fn worker_ring_loop_executes_user_file_io_via_cqe() {
        mudu_sys::task::async_::block_on_tokio_current_thread(async move {
            ...
            assert_eq!(
                mudu_sys::fs::sync::read(&path).unwrap(),
                b"hello iouring".to_vec()
            );
            ...
            mudu_sys::sync::blocking::close_fd(loop_state.mailbox_fd).unwrap();
