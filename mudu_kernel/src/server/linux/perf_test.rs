...
impl AsyncPerfClient {
    ...
    async fn connect_with_loopback_shard(port: u16, shard: usize) -> RS<Self> {
        ...
        let mut last_err: Option<MuduError> = None;
                ...
                .await
            {
                Ok(stream) => {
                    ...
                    if let Err(e) = stream.set_nodelay(true) {
                        return Err(mudu_error!(ErrorCode::Network, "set tcp nodelay error", e));
                    }
                    let mut client = Self {
                        stream,
                        next_request_id: 1,
                ...
                Err(e) => {
                    last_err = Some(mudu_error!(
                        ErrorCode::Network,
                        "connect io_uring tcp server error",
                        e
                    ));
                }
            }
        ...
        Err(last_err.unwrap_or_else(|| {
            mudu_error!(ErrorCode::Network, "connect io_uring tcp server error")
        }))
    }
    ...
    async fn close(mut self) -> RS<()> {
        self.stream.shutdown().await.map_err(|e| {
            mudu_error!(
                ErrorCode::Network,
                "shutdown io_uring perf client stream error",
                e
            )
        })
    }
...
/// Reserves a contiguous block of `count` TCP ports starting outside the
/// ephemeral range. Each worker in multi-port mode binds a distinct port, so
/// reserving the whole block up front prevents `Address already in use` races
/// where worker 0 obtains a free base port but worker 1's offset port is taken.
fn reserve_listener_block(count: usize) -> Option<Vec<StdTcpListener>> {
    if count == 0 {
        return Some(Vec::new());
    }
    ...
    for range in candidate_port_ranges(&ephemeral) {
        for base in range {
            if base
                .checked_add(u16::try_from(count).unwrap_or(u16::MAX).saturating_sub(1))
                .is_none()
            {
                continue;
            }
            ...
            for offset in 0..count {
                let port = base + u16::try_from(offset).expect("offset fits in u16");
            ...
            // Ports in this block are not all free; drop the partially reserved
            // listeners and try the next base port.
        }
    }
    eprintln!(
        "skip io_uring perf test: unable to reserve {} contiguous ports outside ephemeral range",
        count
    );
    None
}
...
fn linux_ephemeral_port_range() -> Option<RangeInclusive<u16>> {
    let raw = mudu_sys::fs::sync::read_to_string("/proc/sys/net/ipv4/ip_local_port_range").ok()?;
...
fn should_skip_iouring_perf(err: &MuduError) -> bool {
...
fn perf_client_setup_timeout(clients: usize) -> Duration {
    let default_secs = std::cmp::max(30, clients.div_ceil(40) as u64);
...
async fn wait_until_server_ready_or_exit_async(port: u16, server: &TestServerHandle) -> RS<()> {
    ...
    while mudu_sys::time::instant_now() < deadline {
        ...
        if let Ok(stream) = AsyncTcpStream::connect(("127.0.0.1", port)).await {
...
fn spawn_iouring_server(
    ...
    let server_launch =
        ServerLaunch::new(server_cfg, server_deps).with_prebound_listeners(listeners);
    let join_handle = mudu_sys::task::sync::spawn_thread(move || {
...
// Miri cannot execute the io_uring syscalls used by this performance test.
#[test]
...
fn iouring_backend_perf_put_get() -> RS<()> {
    ...
    let notifier = NotifyWait::new();
    {
        ...
        let _ = mudu_sys::task::sync::spawn_thread(move || {
            debug_serve(_n, 1800);
        })?;
    };
    let worker_count = 6usize;
    let clients = 6usize;
    let Some(listeners) = reserve_listener_block(worker_count) else {
        return Ok(());
    };
    ...
    mudu_sys::fs::sync::create_dir_all(&data_dir).unwrap();

    let (stop_notifier, server_thread) =
        spawn_iouring_server(listeners, worker_count, &data_dir, 64 * 1024 * 1024, None);
...
// Miri cannot execute the io_uring syscalls used by this recovery test.
#[test]
#[cfg_attr(miri, ignore)]
fn iouring_backend_recovery_replays_worker_logs() -> RS<()> {
    ...
    mudu_sys::task::async_::build_multi_thread_runtime()
        ...
        .block_on(async move {
            ...
            let Some(restart_listeners) = reserve_listener_block(worker_count) else {
                return Ok(());
            };
            let restart_port = restart_listeners[0].local_addr().unwrap().port();
            let (stop_notifier, server_thread) = spawn_iouring_server(
                restart_listeners,
                worker_count,
                &data_dir,
...
fn iouring_backend_recovery_replays_across_multiple_chunks() -> RS<()> {
    mudu_sys::task::async_::build_multi_thread_runtime()
        ...
        .block_on(async move {
            ...
            let (stop_notifier, server_thread) =
                spawn_iouring_server(listeners, worker_count, &data_dir, log_chunk_size, None);
            if let Err(err) = wait_until_server_ready_or_exit_async(port, &server_thread).await {
                if should_skip_iouring_perf(&err) {
            ...
            let chunk_count = mudu_sys::fs::sync::read_dir_entries(&data_dir)
                .unwrap()
                .into_iter()
...
// Miri cannot execute the io_uring syscalls used by this session routing test.
#[test]
#[cfg_attr(miri, ignore)]
fn iouring_backend_open_session_routes_connection_to_requested_partition() -> RS<()> {
    mudu_sys::task::async_::build_multi_thread_runtime()
        ...
        .block_on(async move {
            ...
            let routed_chunk_count = mudu_sys::fs::sync::read_dir_entries(&data_dir)
                .unwrap()
                .into_iter()
                .filter(|entry| {
                    entry
...
// Miri cannot execute the io_uring syscalls used by this session rebind test.
#[test]
#[cfg_attr(miri, ignore)]
fn iouring_backend_open_session_rebind_keeps_same_session_id() -> RS<()> {
    ...
    mudu_sys::task::async_::block_on_async_current(async move {
        ...
        let _guard = lock_network_perf_test();
        trace.watch("rebind_test.stage", "reserve_listener_block");
        let worker_count = 2usize;
        let Some(listeners) = reserve_listener_block(worker_count) else {
            return Ok(());
        };
