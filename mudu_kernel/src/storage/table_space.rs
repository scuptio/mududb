...
impl TableSpaceInner {
    fn flush(&self) -> RS<()> {
        ...
        fs::write(path_buf, json)
            .map_err(|e| m_error!(EC::IOErr, "save table space extent error"))?;
        Ok(())
    }
    ...
    fn new(id: u64, path: String) -> RS<TableSpaceInner> {
        ...
        if path_buf.exists() {
            ...
            fs::create_dir_all(&path_buf).map_err(|e| m_error!(EC::IOErr, "create dir error"))?;
        }
        Ok(Self {
            table_id: id,
            name: "".to_string(),
    ...
    fn allocate_page(&self, ctx: &StorageContext) -> RS<PageIndex> {
        let opt = match self.extent_index_last() {
            Some(index) => ctx.extent_get(&index),
            None => None,
        };
