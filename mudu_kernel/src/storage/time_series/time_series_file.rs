...
use crate::storage::page::page_block_ref::{PAGE_SIZE, PageBlockRef};
...
use crate::wal::pl_batch::{
    NoopPLBatchRecoveryHandler, PLBatch, new_pl_batch_worker_log, new_pl_batch_writer,
};
...
use mudu_sys::default_sys_context;
use mudu_sys::io::fs;
...
impl TimeSeriesFile {
    ...
    pub async fn open_relation_file<P: AsRef<Path>>(
        ...
        scoped_task_trace!();
        Self::open_relation_file_with_sys_context(
            default_sys_context(),
            base_path,
            identity,
            tuple_schema_hash,
    ...
    pub async fn open_relation_file_with_sys_context<P: AsRef<Path>>(
        sys: Arc<SysContext>,
        base_path: P,
        identity: TimeSeriesFileIdentity,
        tuple_schema_hash: u64,
    ...
    async fn open_relation_file_with_fs_and_wal_provider<P: AsRef<Path>>(
        fs: Arc<dyn AsyncFs>,
        wal_provider: Arc<dyn mudu_sys::contract::async_io_provider::AsyncIoProvider>,
        base_path: P,
        identity: TimeSeriesFileIdentity,
        tuple_schema_hash: u64,
        create_if_missing: bool,
    ...
    pub async fn open_ts_file<P: AsRef<Path>>(path: P, create_if_missing: bool) -> RS<Self> {
        Self::open_ts_file_with_sys_context(default_sys_context(), path, create_if_missing).await
    }

    pub async fn open_ts_file_with_sys_context<P: AsRef<Path>>(
        sys: Arc<SysContext>,
        path: P,
        create_if_missing: bool,
    ) -> RS<Self> {
    ...
    async fn open_inner_sync(
        ...
        let file = open_rw(default_sys_context().fs().as_ref(), &path, flags).await?;
        let len = file.file_len().await?;
        if len % PAGE_SIZE as u64 != 0 {
            return Err(m_error!(
                EC::DecodeErr,
    ...
    async fn apply_plan(&mut self, plan: &TimeSeriesFileMutationPlan) -> RS<()> {
        if plan.create_file {
...
async fn open_rw(fs: &dyn AsyncFs, path: &Path, flags: i32) -> RS<SysFile> {
    Ok(SysFile::new(fs.open(path, FileOptions::new(flags, FILE_MODE_644)).await?))
}
...
async fn new_relation_wal_backend_with_provider(
    base_path: &Path,
    identity: &TimeSeriesFileIdentity,
    provider: Arc<dyn mudu_sys::contract::async_io_provider::AsyncIoProvider>,
) -> RS<ChunkedWorkerLogBackend> {
    scoped_task_trace!();
...
async fn recover_relation_file(
    ...
    recover_relation_file_async(default_sys_context().fs(), base_path, identity, backend).await
}
...
impl AsyncWorkerLogRecoveryHandler<PLBatch> for Arc<RelationWalRecoveryHandler> {
    async fn handle_entry(&self, entry: PLBatch, _start_lsn: crate::wal::lsn::LSN) -> RS<()> {
        scoped_task_trace!();
...
#[cfg(test)]
mod tests {
    use super::{PAGE_SIZE, TimeSeriesFile, TimeSeriesFileIdentity};
    use crate::storage::page::PageId;
