...
pub(crate) struct PathCreateDirRequest {
    path: CString,
    mode: u32,
    state: Arc<OpState<()>>,
}

pub(crate) struct PathExistsRequest {
    path: CString,
    statx: Box<libc::statx>,
    state: Arc<OpState<bool>>,
}

pub(crate) struct PathMetadataLenRequest {
    path: CString,
    statx: Box<libc::statx>,
    state: Arc<OpState<u64>>,
}

pub(crate) struct PathRemoveFileRequest {
    path: CString,
    state: Arc<OpState<()>>,
}
...
struct PathCreateDirFuture {
    path: Option<CString>,
    mode: u32,
    state: PathFutureState<()>,
}

struct PathExistsFuture {
    path: Option<CString>,
    state: PathFutureState<bool>,
}

struct PathMetadataLenFuture {
    path: Option<CString>,
    state: PathFutureState<u64>,
}

struct PathRemoveFileFuture {
    path: Option<CString>,
    state: PathFutureState<()>,
}
...
impl PathCreateDirFuture {
    fn new(path: CString, mode: u32) -> Self {
        Self {
            path: Some(path),
            mode,
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        match &self.state {
            PathFutureState::Init => {
                ...
                self.state = PathFutureState::Pending(state);
                self.poll(cx)
            }
...
pub async fn create_dir_all(path: &Path) -> RS<()> {
    if !has_current_worker_ring() {
        return mudu_sys::fs::create_dir_all(path);
    }
...
pub async fn metadata_len(path: &Path) -> RS<u64> {
    if !has_current_worker_ring() {
        return mudu_sys::fs::metadata_len(path);
    }
...
pub(crate) fn complete_path_io(
    ...
    match op {
        PathInflightOp::CreateDir(request) => {
            trace!(path = %request.path().to_string_lossy(), result, "io_path complete mkdirat");
...
fn path_to_cstring(path: &Path) -> RS<CString> {
