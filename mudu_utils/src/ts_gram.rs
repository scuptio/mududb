...
pub fn gen_rs<O: AsRef<Path>, G: AsRef<Path>, M: AsRef<Path>>(
    ...
    let grammar_str = mudu_sys::fs::sync::sync_read_to_string(&grammar_path).unwrap_or_else(|_| panic!("grammar json file path {} cannot be found",
        grammar_path_str));
...
fn grammar_file_changed<P: AsRef<Path>>(s: &String, md5_file: P) -> Option<String> {
    ...
    if !mudu_sys::fs::sync::sync_path_exists(&md5_file) {
...
const COMMENTS: &str = include_str!("text/comments.txt");
