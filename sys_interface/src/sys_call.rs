...
unsafe extern "C" {
    ...
    /// * `out_len`       - Output parameter for minimal required buffer size (mut pointer)
    /// * `mem_id`        - Output parameter for memory identifier when buffer is insufficient (mut pointer)
    ///
    /// # Returns
