...
- Building a `wasm32-wasip2` component and packaging it into a `.mpk` file with `mpk`.
...
1. Run `mgen` to generate Rust entity types from `sql/ddl.sql`.
2. Run `mtp` to transpile the Rust procedures into async generated wrappers.
3. Build the Rust code to a `wasm32-wasip2` component.
4. Run `mpk create` to produce `target/wasm32-wasip2/release/wallet.mpk`.
...
- `package/package.cfg.json` — package metadata for `mpk create`.
