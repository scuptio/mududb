-- MySQL-compatible variant of ddl.sql.
-- The only change from ddl.sql is that TEXT primary keys are converted to
-- VARCHAR so that InnoDB accepts them without an explicit prefix length.
