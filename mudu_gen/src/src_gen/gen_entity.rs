...
fn _gen_from_ddl_sql(
    ...
    for file in input_ddl_path {
        let sql_text =
            mudu_sys::fs::sync::sync_read_to_string(file).map_err(|e| m_error!(EC::IOErr, "open DDL SQL file error", e))?;
    ...
    for (name, source) in gen_result.source_code {
        ...
        mudu_sys::fs::sync::sync_write(file_path, source).map_err(|e| m_error!(EC::IOErr, "write source error", e))?;
    }
